...
SQL=" Select B.bidx, B.buprice From tk_mUnitSub A Join tk_FrmBra B On A.bidx=B.bidx where B.bstatus=1 and A.uidx='"&uidx&"' and B.aidx='"&aidx&"' "
...
Rs2.Close
%>
                                <td class="text-end"><%=FormatNumber(buprice, 0)%></td>
<%
Rs1.movenext
Loop
End if
...
if gubun="insert" then 
    ...
    SQL=SQL&" Values ('"&gtype&"','"&btitle&"', '"&bdepth&"', '"&bwidth&"', '"&bheight&"', '"&bstatus&"', '"&bwidx&"', getdate(), '"&buprice&"', '"&aidx&"')"
    'Response.write (SQL)&"<br>"
    Dbcon.Execute (SQL)	
...
elseif gubun="update" then  
    ...
    SQL=SQL&" , bstatus='"&bstatus&"', bwidx='"&bwidx&"', bwdate=getdate(), buprice='"&buprice&"',aidx='"&aidx&"' "
    SQL=SQL&" where bidx='"&tbidx&"' "
    'Response.write (SQL)&"<br>"
    'response.end
    Dbcon.Execute (SQL)	
