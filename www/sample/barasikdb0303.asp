...
if part="binsert" then 
    ...
    response.write "<script>location.replace('barasik0303.asp');</script>"
    end if 
end if
...
if part="bupdate" then 
    ...
    Response.write (SQL)&"<br>"
    'Dbcon.Execute (SQL)
    response.write "<script>location.replace('barasik0303.asp?rbaidx="&rbaidx&"&part=update');</script>"
end if 
...
'가로세로 최소좌표 최대좌표 구해서 업데이트
    ...
    response.write "<script>location.replace('barasik0303.asp?rbaidx="&rbaidx&"');</script>"
    end if 
