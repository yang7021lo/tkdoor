...
if DDD="delete" then 
    ...
    response.write "<script>location.replace('TNG1_JULGOK_IN_SUB.asp?rbfidx="&rbfidx&"');</script>"
end if
...
if part="binsert" then 
    ...
    SQL="Insert into tk_barasi (baname, bamidx, bawdate, bastatus, bachannel) values ('"&baname&"','"&c_midx&"',getdate(),'"&bastatus&"','"&bachannel&"'  ) "
    Response.write (SQL)&"<br>"
    Dbcon.Execute (SQL)

    response.write "<script>location.replace('TNG1_JULGOK_IN_SUB.asp?rbfidx="&rbfidx&"&rbaidx="&rbaidx&"');</script>"
    end if 
end if
'절곡(tk_barasi) 생성 끝
...
if part="bupdate" then 
    sql="update tk_barasi set baname='"&baname&"' , bastatus='"&bastatus&"' , bachannel='"&bachannel&"' where baidx='"&rbaidx&"' "
    Response.write (SQL)&"<br>"
    'Dbcon.Execute (SQL)
    response.write "<script>location.replace('TNG1_JULGOK_IN_SUB.asp?rbfidx="&rbfidx&"&rbaidx="&rbaidx&"&part=update');</script>"
end if 
...
response.write paccsize&"/<br>"
response.write pbassize&"/<br>"
response.write idv&"/<br>"
...
'가로세로 최소좌표 최대좌표 구해서 업데이트 끝
    ...
    response.write "<script>location.replace('TNG1_JULGOK_BALJU.asp?rbaidx="&rbaidx&"');</script>"
