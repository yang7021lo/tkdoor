...
rsjb_depth= Request("sjb_depth")
rsjb_width= Request("sjb_width")
...
if part="delete" then 
    ...
    response.write "<script>location.replace('TNG1_PUMMOK_Item.asp?kgotopage=" & kgotopage & "&SearchWord="&rSearchWord&"');</script>"
else 
    if rsJB_IDX="0" then 
        SQL = "INSERT INTO TNG_SJB (SJB_TYPE_NO, SJB_TYPE_NAME, SJB_barlist, SJB_FA, SJB_Paint, SJB_St, SJB_Al, SJB_midx, SJB_mdate, SJB_meidx, SJB_medate ,sjb_depth ,sjb_width ) "
        SQL = SQL & "VALUES ('" & rSJB_TYPE_NO & "', '" & rSJB_TYPE_NAME & "', '" & rSJB_barlist & "', '" & rSJB_FA & "', "
        SQL = SQL & rSJB_Paint & ", " & rSJB_St & ", " & rSJB_Al & ", " & C_midx & ", GETDATE(), " & C_midx & ", GETDATE(), '" & rsjb_depth & "', '" & rsjb_width & "'  )"
        'Response.write (SQL)&"<br>"
        'Response.END       
        Dbcon.Execute(SQL)
        ...
        response.write "<script>location.replace('TNG1_PUMMOK_Item.asp?kgotopage=" & kgotopage & "&SJB_IDX="&SJB_IDX&"&SearchWord="&rSearchWord&"#"&SJB_IDX&"');</script>"
    else
        ...
        SQL = SQL & ", SJB_meidx = '" &C_midx & "', SJB_medate = GETDATE() , sjb_depth = '" & rsjb_depth & "', sjb_width = '" & rsjb_width & "' "
        SQL = SQL & " WHERE SJB_IDX = '" & rSJB_IDX & "' "
       ' Response.write(SQL) & "<br>"
         'Response.END
        Dbcon.Execute(SQL)
        response.write "<script>location.replace('TNG1_PUMMOK_Item.asp?kgotopage=" & kgotopage & "&SJB_IDX="&rSJB_IDX&"&SearchWord="&rSearchWord&"#"&SJB_IDX&"');</script>"
    end if
end if
