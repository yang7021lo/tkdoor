...
If gubun = "" Then
    '▼ HTML 출력 부분
    Dim TNG_Busok_idx, bftype, gubun
...
<body>
    ...
    document.addEventListener('paste', function (event) {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].type.indexOf('image') !== -1) {
          ...
          fetch('upload_paste.asp?TNG_Busok_idx=<%=rTNG_Busok_idx%>&bftype=<%=rbftype%>&mode=busok', {
            method: 'POST',
  ...
  onclick="opener.location.replace('TNG1_BUSOK.asp?TNG_Busok_idx=<%=rTNG_Busok_idx%>#<%=rTNG_Busok_idx%>');window.close();">
  창닫기</button>
