...
'SQL=SQL&" and A.sunstatus not in (1,5,6) " 주석처리 이유 al자동문일 경우 미터당 단가일경우 단가 발생해야함
else

SQL=SQL&" and A.sunstatus not in (1,5) "
' 6번 박스커버는 미터당단가에서는 넣기로
    ...
    Rs.close
       ...
       ' 설정 품목 가격 등록 --------------------tk_framekSub 합계금액인데 부속자재는 뺴고
        ...
        sprice=(fprice+taxrate)   '최종금액
        if sprice = 0  or isnull(sprice) then
            sprice = 0  
        end if
        
        'Response.Write "sjsprice_total : " & sjsprice_total & "<br>"  
        'Response.Write "yquan : " & yquan & "<br>"  
