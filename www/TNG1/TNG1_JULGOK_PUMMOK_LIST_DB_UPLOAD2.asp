...
<body>
    ...
    document.addEventListener('paste', function (event) {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].type.indexOf('image') !== -1) {
          ...
          fetch('upload_paste.asp?sjb_idx=<%=rsjb_idx%>&bfidx=<%=rbfidx%>&bftype=<%=rbftype%>', {
            method: 'POST',
  ...
  <button type="button" class="btn btn-outline-danger" onclick="opener.location.replace('TNG1_JULGOK_PUMMOK_LIST1.asp?bfidx=<%=rbfidx%>&SJB_IDX=<%=rSJB_IDX%>');window.close();">창닫기</button>
