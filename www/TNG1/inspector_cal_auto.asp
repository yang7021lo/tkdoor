...
if rsjb_type_no > 0  and rqtyidx > 0   and  rtw > 0 and rth > 0  then
        ...
        Select Case CInt(rsjb_type_no) '도어포함견적이냐 별도견적이냐 구분
...
'Response.Write "doorsizechuga_price: " & doorsizechuga_price & "<br>"
        ...
        End If 
        If rsjb_type_no = 1  Then  ' 알자 sidx
        SQL=SQL&" WHERE sidx=31   "
        End If 
        If rsjb_type_no = 2  Then  ' 복층알자
        SQL=SQL&" WHERE sidx=32  "
        End If 
        If rsjb_type_no = 3 or  rsjb_type_no = 4 or rsjb_type_no = 8 or rsjb_type_no = 9 or rsjb_type_no = 10 or rsjb_type_no = 15   Then  ' 단열알자3 삼중알자4 단자8 삼중단자9 이중슬라이딩10 포켓15
        SQL=SQL&" WHERE sidx=106  " 
        End If 
        If rsjb_type_no = 5  Then  ' 인테
        SQL=SQL&" WHERE sidx=33   " 
        End If 
        If rsjb_type_no = 6  Then  ' 통도장 수동 일반
        SQL=SQL&" WHERE sidx=303 " 
        End If 
        If rsjb_type_no = 7  Then  ' 통도장 수동 단열
        SQL=SQL&" WHERE sidx=322 " 
        End If 
        If rsjb_type_no = 11  Then  ' 수동 단열
        SQL=SQL&" WHERE sidx=376  " 
        End If 
        If rsjb_type_no = 12 Then  ' 수동 단열
...
'Response.Write "g_h: " & g_h & "<br>"
        ...
        Rs1.close      
                ...
                elseif ( rqtyidx =15 or  rqtyidx =30)  Then '알미늄블랙 5 실버15 기타도장30
                    if rsjb_type_no = 7  then
                        dojang_price=30000
                    elseif  rsjb_type_no = 6  then
                        dojang_price=20000  
                    end if
                else
                    dojang_price=0
                ...
                if rdooryn = 2 Then ' 0 도어나중 1 도어같이 2 도어안함
                door_price=0
                doorsizechuga_price=0
                end if
...
'=========================================
 ...
 ' 설정 도장 2코트 쓰리코드  가격 등록  coat=0 기본 coat=1 2코트 coat=2 3코트
    SQL = "SELECT coat FROM tk_paint "
