...
%>
    ...
    <script>
      function sendDimensions(tw, th) {
        ...
        const url = `choiceframeb.asp?mode=twth&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&greem_f_a=<%=rgreem_f_a%>&sjb_type_no=<%=rsjb_type_no%>&tw=${encodeURIComponent(tw)}&th=${encodeURIComponent(th)}`;
...
<script>onSendButtonClick();</script>
  ...
  response.write rtw&"/<br>"
      'yi 갯수 찾기
      SQL="Select distinct yi From tk_framekSub where fkidx='"&rfkidx&"' order by yi asc"
      Response.write (SQL)&"<br><br>"
      Rs.open Sql,Dbcon
      If Not (Rs.bof or Rs.eof) Then 
      Do while not Rs.EOF
        ...
        Response.write "#"&gcnt&"/<br>"
          ...
          SQL=" Select A.fksidx, A.xi, A.yi, A.wi, A.hi, A.bfidx, A.blength, A.xsize, A.ysize , A.whichi_fix, A.whichi_auto "
          SQL=SQL&" From tk_FramekSub A "
          SQL=SQL&" Where A.fkidx='"&rfkidx&"' and A.yi='"&yi&"' "
          Response.write (SQL)&"<br><br>"
          Rs1.open Sql,Dbcon
          ...
          Do while not Rs1.EOF
            ...
            whichi_auto=Rs1(10)
              ...
              Else
                  whrate = 0
              End If
              Response.Write "퍼센트: " & Round(whrate, 2) & "%<br>"

              If whrate < 50 Then '세로바라면
          ...
          'SQL="Select sum(ysize) From tk_framekSub Where fkidx='"&rfkidx&"'  and yi<='"&yi&"' and hi>'"&yi&"'-yi "
          'if whichi_fix="12" or whichi_fix="14" or whichi_fix="16" then 
          'SQL=SQL&" and whichi_fix not in (1, 4, 5, 12, 14, 16) "
          'else
          'SQL=SQL&" and whichi_fix not in (1, 4, 5) "
          'end if 
          ...
          if ctw>1 then 
            ...
            if whichi_fix="12" or whichi_fix="13" or whichi_fix="14" or whichi_fix="15" or whichi_fix="16" then 
            SQL=SQL&" and whichi_fix not in (1, 4, 5, 12, 13, 14, 15, 16) "
            else
            SQL=SQL&" and whichi_fix not in (1, 4, 5) "
            end if 
...
' ▶ glassselect, whichi_auto, whichi_fix에 따른 barasiF 조회
    ...
    Else
        If whichi_auto <> "" Then ' glassselect ≠ 0일 때 자동유리
            SQL = "SELECT bfidx, xsize, ysize "
            SQL = SQL & "FROM tk_barasiF "
            SQL = SQL & "WHERE sjb_idx = '129'"
            SQL = SQL & " AND whichi_auto = '" & whichi_auto & "'"
            Response.Write(SQL) & "  ---[glassselect ≠ 0 and whichi_auto]<br><br>"
        ElseIf whichi_fix <> "" Then ' glassselect ≠ 0일 때 수동유리
            SQL = "SELECT bfidx, xsize, ysize "
            SQL = SQL & "FROM tk_barasiF "
...
If IsNumeric(glassselect_auto) And glassselect_auto <> "" Then
    gls = glassselect_auto
ElseIf IsNumeric(glassselect_fix) And glassselect_fix <> "" Then
    gls = glassselect_fix
End If
...
response.write "<script>location.replace('choiceframeb.asp?sjidx="&rsjidx&"&sjsidx="&rsjsidx&"&sjb_idx="&rsjb_idx&"&sjb_type_no="&rsjb_type_no&"&greem_f_a="&rgreem_f_a&"&fkidx="&rfkidx&"&mode="&rmode&"');alert('추가되었습니다.');opener.location.replace('TNG1_B_suju_temp.asp?sjidx="&rsjidx&"&sjsidx="&rsjsidx&"&sjb_type_no="&rsjb_type_no&"&sjb_idx="&rsjb_idx&"&fkidx="&fkidx&"');</script>"
End If
'=======================================
...
<html lang="en">
  ...
  <script>
    ...
    function pummoksub(fidx){
        ...
        {
            location.href="choiceframeb.asp?part=pummoksub&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_type_no=<%=rsjb_type_no%>&sjb_idx=<%=rsjb_idx%>&greem_f_a=<%=rgreem_f_a%>&fkidx=<%=rfkidx%>&mode=<%=rmode%>&fidx="+fidx;
        }
    }
  </script>
...
<form name="frmMainsub" action="choiceframeb.asp" method="POST">  
