...
rsearchword=Request("SearchWord")
	...
	page_name="TNG1_PUMMOK_Item_gd.asp?SearchWord="&Request("SearchWord")&"&"
  ...
  <head>
    ...
    <script>
        ...
        function del(sTR){
            ...
            {
                location.href="TNG1_PUMMOK_Item_gddb.asp?part=delete&searchWord=<%=rsearchword%>&SJB_IDX="+sTR;
            }
        }
    </script>
</head>
    ...
    <div class="container-fluid px-4">
      ...
      <div class="row">
        ...
        <div class="col-3 text-start">
          <form id="Search" name="Search" action="tng1_pummok_item_gd.asp" method="POST">   
          <!-- input 형식 시작--> 
                  <div class="input-group mb-3">
                      ...
                      <input type="text" class="form-control" name="SearchWord" value="<%=Request("SearchWord")%>">
                        ...
                        onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?SJB_IDX=0');">등록</button>
                  </div>
          <!-- input 형식 끝--> 
        ...
        <div>
            <div style="width: 100%; margin: 0; padding: 0;">
                <table style="width: 100%; border-collapse: collapse;" id="datatablesSimple"  class="table table-hover">
                    ...
                    <tbody>
                        <form id="dataForm" name="dataForm" action="TNG1_PUMMOK_Item_gddb.asp" method="POST" >   
                            ...
                            <tr bgcolor="<%=cccc%>" >
                                ...
                                <select class="input-field" name="SJB_TYPE_NO" id="SJB_TYPE_NO"  onchange="handleChange(this)">
                                        <%
                                        If SJB_TYPE_NO = "" or IsNull(SJB_TYPE_NO)  Then SJB_TYPE_NO = "14"
                                        ...
                                        %>
                                        <option value="<%=ySJB_TYPE_NO%>" <% If cint(SJB_TYPE_NO) = cint(rSJB_TYPE_NO) Then Response.Write "selected" End If %> >
                                            <%=ySJB_TYPE_NAME%>
                                        </option>
                                        <%
                                        Rs1.MoveNext
                                    ...
                                    <!-- 인서트용 팝업 버튼 -->
                                        ...
                                        onclick="window.open('TNG1_whichi_INSERT.asp?gotopage=<%=gotopage%>&SJB_IDX=<%=SJB_IDX%>&SearchWord=<%=rSearchWord%>#<%=SJB_IDX%>', 'typeInsert', 'width=500,height=300,scrollbars=no');">
                                        + 추가
                                    </button>
                                </td>
                                <td>
                                ...
                                <td>
                                    <select class="input-field" name="SJB_Paint" id="SJB_Paint"  onchange="handleSelectChange(event, 'SJB_Paint', 'SJB_Paint')">
                                        <option value="0" <% If SJB_Paint = "0" Then Response.Write "selected" %> >안함</option>
                                        <option value="1" <% If SJB_Paint = "1" Then Response.Write "selected" %> >사용</option>
                                    </select>
                                </td> 
                                <td>
                                    <select class="input-field" name="SJB_St" id="SJB_St"  onchange="handleSelectChange(event, 'SJB_St', 'SJB_St')">
                                        <option value="0" <% If SJB_St = "0" Then Response.Write "selected" %> >안함</option>
                                        <option value="1" <% If SJB_St = "1" Then Response.Write "selected" %> >사용</option>
                                    </select>
                                </td> 
                                <td>
                                    <select class="input-field" name="SJB_Al" id="SJB_Al"  onchange="handleSelectChange(event, 'SJB_Al', 'SJB_Al')">
                                        <option value="0" <% If SJB_Al = "0" Then Response.Write "selected" %> >안함</option>
                                        <option value="1" <% If SJB_Al = "1" Then Response.Write "selected" %> >사용</option>
                                    </select>
                                </td> 
                                -->
                                <td></td> <!-- 작성자 키 -->
                            ...
                            <% 
                                ...
                                End If
                                SQL = SQL & " ORDER BY A.SJB_IDX DESC"
                                'Response.write (SQL)&"<br>"
                            ...
                            if int(SJB_IDX)=int(rSJB_IDX) then 
                            cccc="#E7E7E7"
                            %>
                            <tr bgcolor="<%=cccc%>">
                                <td align="center"><a name="<%=SJB_IDX%>">-><button type="button" class="btn btn-outline-danger" Onclick="del('<%=SJB_IDX%>');"><%=no-j%></button></td> <!-- 삭제  -->
                                <td>
                                    <select class="input-field" name="SJB_TYPE_NO" id="SJB_TYPE_NO"  onchange="handleChange(this)">
                                        <%
                                ...
                                <td>
                                    <button class="btn btn-success btn-small" type="button" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST1GD.asp?SJB_IDX=<%=rSJB_IDX%>#<%=rSJB_IDX%>');">유리도어등록</button>
                                </td>
                                <td>
                                    <select class="input-field" name="SJB_FA" id="SJB_FA"  onchange="handleSelectChange(event, 'SJB_FA', 'SJB_FA')">
                                        <option value="0" <% If SJB_FA = "0" Then Response.Write "selected" %> >안함</option>
                                ...
                                <td>
                                    <select class="input-field" name="SJB_Paint" id="SJB_Paint"  onchange="handleSelectChange(event, 'SJB_Paint', 'SJB_Paint')">
                                        <option value="0" <% If SJB_Paint = "0" Then Response.Write "selected" %> >안함</option>
                                        <option value="1" <% If SJB_Paint = "1" Then Response.Write "selected" %> >사용중</option>
                                    </select>
                                </td> 
                                <td>
                                    <select class="input-field" name="SJB_St" id="SJB_St"  onchange="handleSelectChange(event, 'SJB_St', 'SJB_St')">
                                        <option value="0" <% If SJB_St = "0" Then Response.Write "selected" %> >안함</option>
                                        <option value="1" <% If SJB_St = "1" Then Response.Write "selected" %> >사용중</option>
                                    </select>
                                </td> 
                                <td>
                                    <select class="input-field" name="SJB_Al" id="SJB_Al"  onchange="handleSelectChange(event, 'SJB_Al', 'SJB_Al')">
                                        <option value="0" <% If SJB_Al = "0" Then Response.Write "selected" %> >안함</option>
                                        <option value="1" <% If SJB_Al = "1" Then Response.Write "selected" %> >사용중</option>
                                    </select>
                                </td> 
                                -->
                            ...
                            %>
                            <tr bgcolor="<%=cccc%>">
                                <td align="center"><%=no-j%><a name="<%=SJB_IDX%>"></td><!-- 순번 -->
                                <td><input class="input-field" type="text" value="<%=ySJB_TYPE_NAME%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td>                           
                                <td style="white-space: nowrap;">
                                    <input class="input-field" type="text" value="<%=SJB_barlist%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');" />
                                </td>
                                <td>
                                    <button class="btn btn-success btn-small" type="button" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST1GD.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');">유리도어등록</button>
                                </td>
                                <td><input class="input-field" type="text" value="<%=SJB_FA_text%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td>  
                                <!--
                                <td><input class="input-field" type="text" value="<%=SJB_Paint_text%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td>  
                                <td><input class="input-field" type="text"  value="<%=SJB_St_text%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td> 
                                <td><input class="input-field" type="text"  value="<%=SJB_Al_text%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td> 
                                -->
                                <td><input class="input-field" type="text"  value="<%=mename%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/></td> <!-- 최종 수정자 키 -->
                                <td><input class="input-field" type="text"  value="<%=SJB_medate%>" onclick="location.replace('TNG1_PUMMOK_Item_gd.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/></td> <!-- 최종 수정일시 -->
                            </tr>
                            <% end if %>
                            <%
