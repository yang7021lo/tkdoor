...
'Response.end
	...
	page_name="unittype.asp?SearchWord="&Request("SearchWord")&"&"
    ...
    <div class="container-fluid px-4">
        <div class="row justify-content-between  mt-2">
            <div class="py-5 container text-center">
                    ...
                    <!-- 제목 나오는 부분 끝-->
                    <div class="row">
<form id="dataForm" action="unittypedb_p.asp" method="post">
    <div class="row justify-content-between  mt-2">
    <div class="col-4" style="border:1px solid #ccc; padding:5px; border-radius:4px;">
    <h5 class="text-primary">■ 1차 (X45)</h5>
    <div class="row justify-content-between mt-2">
        ...
        SQL = SQL & " WHERE A.SJB_FA = 1 "
        'SQL = SQL & " AND A.SJB_barlist='120X45' "
        'SQL = SQL & " AND right(A.SJB_barlist,2)='45' "
        SQL = SQL & " AND A.SJB_barlist LIKE '%45%' "
        ...
        <% if cint(SJB_IDX)=cint(rSJB_IDX)  then %>
            ...
            <div class="row justify-content-between  mt-2">
                ...
                <div class="col-3">
                    ...
                    onclick="window.open('unittype_new.asp?SJB_IDX=<%=SJB_IDX%>&SJB_TYPE_No=<%=SJB_TYPE_No%>&SJB_barlist=<%=SJB_barlist%>&pcent=<%=pcent%>&bar=<%=bar%>', 'popup', 'width=1920,height=600,scrollbars=yes');" />
                </div>
                <div class="col-3">
                ...
                <% if pcent=1 then %>
                    <button type="button" class="btn btn-primary btn-sm"
                        onclick="window.open('unittype_l.asp?SJB_IDX=<%=SJB_IDX%>&SJB_TYPE_No=<%=SJB_TYPE_No%>&SJB_barlist=<%=SJB_barlist%>&pcent=<%=pcent%>&bar=<%=bar%>', 'pcentWindow', 'width=1200,height=700,scrollbars=yes');">
                        pcent_insert
                    </button>
                <% end if %> 
                <% if pcent=0 then %>
        ...
        <% else %> 
            <div class="row justify-content-between  mt-2">  
                ...
                <div class="col-3">
                    <input class="form-control" type="text" value="<%=SJB_TYPE_NAME%>" onclick="location.replace('unittype_p.asp?SJB_IDX=<%=SJB_IDX%>&SJB_TYPE_No=<%=SJB_TYPE_No%>&SJB_barlist=<%=SJB_barlist%>&pcent=<%=pcent%>&bar=<%=bar%>');" />
                </div>
                <div class="col-3">
                    <input class="form-control" type="text" value="<%=SJB_barlist%>" onclick="location.replace('unittype_p.asp?SJB_IDX=<%=SJB_IDX%>&SJB_TYPE_No=<%=SJB_TYPE_No%>&SJB_barlist=<%=SJB_barlist%>&pcent=<%=pcent%>&bar=<%=bar%>');" />
                </div>
                <div class="col-3">
                    <input class="form-control" type="text" value="<%=pcent%>" onclick="location.replace('unittype_p.asp?SJB_IDX=<%=SJB_IDX%>&SJB_TYPE_No=<%=SJB_TYPE_No%>&SJB_barlist=<%=SJB_barlist%>&pcent=<%=pcent%>&bar=<%=bar%>');" />
                </div>
                <div class="col-3">
                <% if pcent=1 then %>
                    <button type="button" class="btn btn-primary btn-sm"
        ...
        <div class="col-4" style="border:1px solid #ccc; padding:5px; border-radius:4px;">
            <h5 class="text-success">■ 2차 (X60)</h5>
            <div class="row justify-content-between mt-2">
                <div class="col-3"><strong>품명</strong></div>
                <div class="col-3"><strong>규격</strong></div>
                <div class="col-3"><strong>프로티지</strong></div>
        ...
        SQL = SQL & " LEFT OUTER JOIN tng_sjbtype D ON A.SJB_TYPE_NO = D.SJB_TYPE_NO AND D.sjbtstatus = 1 "
        SQL = SQL & " WHERE A.SJB_FA = 1 "
        SQL = SQL & " AND right(A.SJB_barlist,3)='X60' "
        SQL = SQL & " AND A.SJB_TYPE_NO <> '12' "
        ...
        SQL = SQL & " LEFT OUTER JOIN tng_sjbtype D ON A.SJB_TYPE_NO = D.SJB_TYPE_NO AND D.sjbtstatus = 1 "
        SQL = SQL & " WHERE A.SJB_FA = 1 "
        sql = sql & " AND A.SJB_TYPE_NO<>'6' "
        SQL = SQL & " AND A.SJB_TYPE_NO<>'7' "
        SQL = SQL & " AND A.SJB_TYPE_NO<>'11' "
        SQL = SQL & " ORDER BY D.sjbtidx ASC, A.SJB_IDX aSC"
        'response.write (Sql)&"<br>"
        Rs.open Sql,Dbcon,1,1,1
        if not (Rs.EOF or Rs.BOF ) then
