...
rsearchword=Request("SearchWord")
	...
	page_name="TNG1_PUMMOK_Item.asp?SearchWord="&Request("SearchWord")&"&"
  ...
  <head>
    ...
    <script>
        ...
        function del(sTR){
            ...
            {
                location.href="TNG1_PUMMOK_ItemDB.asp?part=delete&searchWord=<%=rsearchword%>&SJB_IDX="+sTR;
            }
        }
    </script>
</head>
          ...
          <!-- input 형식 시작--> 
                  <div class="input-group mb-3">
                      ...
                      <input type="text" class="form-control" name="SearchWord" value="<%=Request("SearchWord")%>">
                        ...
                        onclick="location.replace('TNG1_PUMMOK_Item.asp?SJB_IDX=0');">등록</button>
                  </div>
          <!-- input 형식 끝--> 
        ...
        <div>
            <div style="width: 100%; margin: 0; padding: 0;">
                <table style="width: 100%; border-collapse: collapse;" id="datatablesSimple"  class="table table-hover">
                    ...
                    <tbody>
                        <form id="dataForm" name="dataForm" action="TNG1_PUMMOK_ItemDB.asp" method="POST" >   
                            ...
                            <tr bgcolor="<%=cccc%>" >
                                ...
                                <select class="input-field" name="SJB_TYPE_NO" id="SJB_TYPE_NO"  onchange="handleChange(this)">
                                    ...
                                    <!-- 인서트용 팝업 버튼 -->
                                        ...
                                        onclick="window.open(
                                        'TNG1_SJB_TYPE_INSERTgl.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&SearchWord=<%=rSearchWord%>#<%=SJB_IDX%>', 
                                        'typeInsert', 
                                ...
                                <td>
                                    <input class="input-field" type="text"  placeholder="폭(깊이)" name="sjb_depth" id="sjb_depth" value="<%=sjb_depth%>" onkeypress="handleKeyPress(event, 'sjb_depth', 'sjb_depth')"/>
                                </td>
                                <td>
                                    <input class="input-field" type="text"  placeholder="보이는면"  name="sjb_width" id="sjb_width" value="<%=sjb_width%>" onkeypress="handleKeyPress(event, 'sjb_width', 'sjb_width')"/>
                                </td>
                                <td></td> 
                                <td>
                            ...
                            <% 
                                ...
                                SQL = SQL & ", B.mname, C.mname, A.SJB_FA ,a.sjb_depth,a.sjb_width "
                            ...
                            <tr bgcolor="<%=cccc%>">
                                ...
                                <td>
                                    <button class="btn btn-success btn-small" type="button" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST1.asp?SJB_IDX=<%=rSJB_IDX%>#<%=rSJB_IDX%>');">자재등록수정</button>
                                </td>
                                <td>
                                    <select class="input-field" name="SJB_FA" id="SJB_FA"  onchange="handleSelectChange(event, 'SJB_FA', 'SJB_FA')">
                                        <option value="0" <% If SJB_FA = "0" Then Response.Write "selected" %> >❌</option>
                            ...
                            %>
                            <tr bgcolor="<%=cccc%>">
                                <td align="center"><%=SJB_IDX%><a name="<%=SJB_IDX%>"></td><!-- 순번 -->
                                <td><input class="input-field" type="text" value="<%=ySJB_TYPE_NAME%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?gotopage=<%=gotopage%>&kgotopage=<%=kgotopage%>&searchword=<%=rsearchword%>&ksearchword=<%=rksearchword%>&SJB_IDX=<%=SJB_IDX%>');"/> </td>                           
                                <td style="white-space: nowrap;">
                                    <input class="input-field" type="text" value="<%=SJB_barlist%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');" />
                                </td>
                                
                                <td><input class="input-field" type="text" value="<%=sjb_depth%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td>  
                                <td><input class="input-field" type="text" value="<%=sjb_width%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td>  
                                <td>
                                    <button class="btn btn-success btn-small" type="button" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST1.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');">자재등록보기</button>
                                </td>
                                <td><input class="input-field" type="text" value="<%=SJB_FA_text%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td>  
                                <td><input class="input-field" type="text" value="<%=SJB_Paint_text%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td>  
                                <td><input class="input-field" type="text"  value="<%=SJB_St_text%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td> 
                                <td><input class="input-field" type="text"  value="<%=SJB_Al_text%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/> </td> 
                                <td><input class="input-field" type="text"  value="<%=mename%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/></td> <!-- 최종 수정자 키 -->
                                <td><input class="input-field" type="text"  value="<%=SJB_medate%>" onclick="location.replace('TNG1_PUMMOK_Item.asp?kgotopage=<%=kgotopage%>&SJB_IDX=<%=SJB_IDX%>&searchWord=<%=rsearchword%>#<%=SJB_IDX%>');"/></td> <!-- 최종 수정일시 -->
                            </tr>
                            <% end if %>
                            <%
