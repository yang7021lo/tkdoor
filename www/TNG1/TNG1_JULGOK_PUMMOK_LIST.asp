  ...
  <head>
    ...
    <script>
        ...
        function del(sTR){
            ...
            {
                location.href="TNG1_JULGOK_PUMMOK_LIST_DB.asp?part=delete&bfidx="+sTR;
            }
        }
    </script>
    <script>
    ...
    <div class="container-fluid px-4">
        ...
        <div class="row justify-content-between">
            ...
            <div class="col text-end">
                <form class="form-inline ms-auto me-0 -3 my-2 my-md-0" method="post" action="TNG1_JULGOK_PUMMOK_LIST.asp" name="form1">   
                  ...
                  <input type="hidden" name="SJB_IDX" value="<%=rSJB_IDX%>">
                    ...
                    <button type="button"
                        ...
                        onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?bfidx=0&SJB_IDX=<%=rSJB_IDX%>');">등록
                    </button>
                </div>
            </div>
        <div>
                ...
                <table style="width: 100%; border-collapse: collapse;" id="datatablesSimple"  class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center;">절곡바라시</th>
                            <th style="text-align: center;">측면폭</th>
                            <th style="text-align: center;">정면폭</th>
                            <th style="text-align: center;" width="200">알루미늄자재</th>
                    ...
                    <tbody>
                        <form id="dataForm" action="TNG1_JULGOK_PUMMOK_LIST_DB.asp" method="POST" >   
                            ...
                            <tr bgcolor="<%=cccc%>" >
                                ...
                                <td><input class="input-field" type="text" size="" name="set_name_AUTO" id="set_name_AUTO" value="<%=set_name_AUTO%>" onkeypress="handleKeyPress(event, 'set_name_AUTO', 'set_name_AUTO')"/></td> 
                                <td>
                                ...
                                <td><!--
                                    <button 
                                        ...
                                        type="button"
                                        style="width: 80px;"  
                                        onclick="window.open('tng1_julgok_in_sub3.asp?SJB_IDX=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>', 'popupWindow', 'width=1200,height=800,scrollbars=yes,resizable=yes');">
                                        절곡바라시
                                    </button>
                                    -->
                                </td>
                                ...
                                <td><input class="input-field" type="text"  name="bfimg1" id="bfimg1" value="<%=bfimg1%>" onkeypress="handleKeyPress(event, 'bfimg1', 'bfimg1')"/><%=bfimg1%></td> <!-- 이미지파일 -->
                                <td><input class="input-field" type="text"  name="bfimg2" id="bfimg2" value="<%=bfimg2%>" onkeypress="handleKeyPress(event, 'bfimg2', 'bfimg2')"/><%=bfimg2%></td> <!-- 이미지파일 -->
                                <td></td> <!-- 작성자 키 -->
                                <td></td> <!-- 최초 작성일 -->
                                <td></td> <!-- 최종 수정자 키 -->
                                <td></td> <!-- 최종 수정일시 -->
                            ...
                            cccc=""
                                ...
                                SQL = SQL & " , A.bfemidx, Convert(varchar(10), A.bfewdate, 121), B.mname, C.mname , A.TNG_Busok_idx, D.T_Busok_name_f "
                            ...
                            %>
                            <tr bgcolor="<%=cccc%>">
                                <td align="center"><!--<a name="<%=bfidx%>">--><button type="button" class="btn btn-outline-danger" Onclick="del('<%=bfidx%>');"><%=i%></button></td> <!-- 삭제  -->
                                <td><input class="input-field" type="text" size="" name="set_name_FIX" id="set_name_FIX" value="<%=set_name_FIX%>" onkeypress="handleKeyPress(event, 'set_name_FIX', 'set_name_FIX')"/></td> 
                                <td>
                                    <select class="input-field" name="WHICHI_FIX" id="WHICHI_FIX"  onchange="handleChange(this)">
                                        <option value="0" <% If WHICHI_FIX = "0" Then Response.Write "selected" %> >없음</option>
...
%>
                                ...
                                <td><% if bfimg1<>"" then %><img src="/img/frame/bfimg/<%=bfimg1%>" width="150" height="150" onclick="window.open('TNG1_JULGOK_PUMMOK_LIST_DB_UPLOAD.asp?rsjb_idx=<%=sjb_idx%>&rbfidx=<%=bfidx%>&rbftype=bfimg1','bfimg1','top=10, left=10, width=700, height=200');"><% else %><button class="btn btn-primary btn-small " type="button"  onclick="window.open('TNG1_JULGOK_PUMMOK_LIST_DB_UPLOAD.asp?rsjb_idx=<%=sjb_idx%>&rbfidx=<%=bfidx%>&rbftype=bfimg1','bfimg1','top=10, left=10, width=700, height=200');" >등록</button><% end if %></td> <!-- 이미지파일 -->
                                <td><% if bfimg2<>"" then %><img src="/img/frame/bfimg/<%=bfimg2%>" width="150" height="150" onclick="window.open('TNG1_JULGOK_PUMMOK_LIST_DB_UPLOAD.asp?rsjb_idx=<%=sjb_idx%>&rbfidx=<%=bfidx%>&rbftype=bfimg2','bfimg2','top=10, left=10, width=700, height=200');"><% else %><button class="btn btn-primary btn-small " type="button"  onclick="window.open('TNG1_JULGOK_PUMMOK_LIST_DB_UPLOAD.asp?rsjb_idx=<%=sjb_idx%>&rbfidx=<%=bfidx%>&rbftype=bfimg2','bfimg2','top=10, left=10, width=700, height=200');" >등록</button><% end if %></td> <!-- 이미지파일 -->
                                <td><%=mname%></td> <!-- 작성자 키 -->
                                <td><%=bfwdate%></td> <!-- 최초 작성일 -->
                                <td><%=mename%></td> <!-- 최종 수정자 키 -->
                                <td><%=bfewdate%></td> <!-- 최종 수정일시 -->
                            </tr>
                            <% else 
                            'cccc="#CCCCCC"
                            %>
                            <tr bgcolor="<%=cccc%>">
                                <td align="center"><%=i%><a name="<%=bfidx%>"></td><!-- 순번 -->
                                <td><input class="input-field" type="text" value="<%=set_name_FIX%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/> </td>  
                                <td><input class="input-field" type="text" value="<%=WHICHI_FIX_text%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/> </td>                           
                                <td><input class="input-field" type="text" value="<%=set_name_AUTO%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/> </td>  
                                <td><input class="input-field" type="text" value="<%=WHICHI_AUTO_text%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/> </td>                           
                                <td>
                                    <button 
                                        class="btn btn-success btn-small" 
                                        type="button"
                                ...
                                <td>
                                    <input class="input-field" type="text" size="8" value="<%=xsize%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/> 
                                </td>  
                                <td><input class="input-field" type="text" size="8" value="<%=ysize%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/> </td> 
                                <td><input class="input-field" type="text" value="<%=T_Busok_name_f%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/></td>
                                <td><% if bfimg1<>"" then %><img src="/img/frame/bfimg/<%=bfimg1%>" width="150" height="150" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"><% end if %></td> 
                                <td><% if bfimg2<>"" then %><img src="/img/frame/bfimg/<%=bfimg2%>" width="150" height="150" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"><% end if %></td> 
                                <td><input class="input-field" type="text"  value="<%=mname%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/></td> <!-- 작성자 키 -->
                                <td><input class="input-field" type="text"  value="<%=bfwdate%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/></td> <!-- 최초 작성일 -->
                                <td><input class="input-field" type="text"  value="<%=mename%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/></td> <!-- 최종 수정자 키 -->
                                <td><input class="input-field" type="text"  value="<%=bfewdate%>" onclick="location.replace('TNG1_JULGOK_PUMMOK_LIST.asp?sjb_idx=<%=SJB_IDX%>&bfidx=<%=bfidx%>#<%=bfidx%>');"/></td> <!-- 최종 수정일시 -->
                            </tr>
                            <% end if %>
                            <%
                            set_name_FIX_text =""
