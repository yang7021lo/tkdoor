...
if rafksidx<>"" then 
  ...
  response.write "<script>location.replace('TNG1_B_suju4.asp?cidx="&rcidx&"&sjidx="&rsjidx&"&sjsidx="&rsjsidx&"')</script>"
end if
'===================
'바 추가 복제하기 끝
'개별 바의 길이 입력시작
...
If Request("new_open") = "go" Then
    ...
    response.write "window.open('TNG1_B_suju4_pop.asp?cidx=" & rcidx & _
        "&sjidx=" & rsjidx & _
        "&sjsidx=" & rsjsidx & _
        "&fkidx=" & fkidx & _
        "&sjb_idx=" & rsjb_idx & _
...
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
  <script>
      ...
      if (message !== null && message.trim() !== "") {
        ...
        window.location.href = "TNG1_B_suju4.asp?part=pummoksub&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx="+sjb_idx+"&fknickname="+encodedMessage;
      }
    }
    function framedel(fkidx){
        if (confirm("프레임을 삭제 하시겠습니까?"))
        {
            location.href="TNG1_B_suju4.asp?part=framedel&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx="+fkidx;
        }
    }
    function bardel(fksidx){
        if (confirm("바를 삭제 하시겠습니까?"))
        {
            location.href="TNG1_B_suju4.asp?part=bardel&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx="+fksidx;
        }
    }
    function setstd(ni,fkidx){

        {
            location.href="TNG1_B_suju4.asp?part=setstd&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx="+fkidx+"&ni="+ni;
        }
    }
    function wresize(order){
        {
            location.href="TNG1_B_suju4.asp?part=wresize&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&order="+order;
        }
    }
    function hresize(order){
        {
            location.href="TNG1_B_suju4.asp?part=hresize&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&order="+order;
        }
    }
    function converge(direction){
        {
            location.href="TNG1_B_suju4.asp?part=converge&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&direction="+direction;
        }
    }
    
    function addglass(glasstype, wsize, hsize){
        {
            location.href="TNG1_B_suju4.asp?part=addglass&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&gubun=glass&glasstype="+glasstype+"&wsize="+wsize+"&hsize="+hsize;
        }
    }
 
    function addcovered(alocation){
        {
            location.href="TNG1_B_suju4.asp?part=addcovered&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&gubun=covered&alocation="+alocation;
        }
    }
    function wincopy(){
...
<%
                ...
                SQL = "SELECT MAX(fl) FROM tk_framek WHERE sjsidx = '" & rsjsidx & "'"
                Rs3.open Sql,Dbcon
...
<form id="dataForm_original" name="dataForm_original"  action="TNG1_B_suju_sjasub.asp" method="POST" >   
    ...
    <input type="hidden" name="bury_door" value="<%=bury_door%>">
        ...
        <div class="second-left-1"> <!-- 첫 번째 영역 -->
            ...
            <div class="mb-2 d-flex align-items-center " >
                ...
                <!-- 드롭다운 버튼 끝-->
                 <div >
                        <% if rfkidx<>"" then %>
<button class="btn btn-secondary  btn-small" type="button" Onclick="window.open('modal/calculator/dr-gls?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>','TNG1_B_doorpop','top=100 left=400 width=1000 height=800');">도어유리보기</button>
                        <% end if%> 
                    </div>
            </div>
                <!-- 생성된 도면 정보 시작 -->
                <div class="row">
                    <div class="col-9">
                        ...
                        <div class="input-group mb-1">     
                            ...
                            onclick="location.replace('TNG1_B_suju4.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>');" 
                ...
                <div class="mb-2">
                    ...
                    <div class="input-group mb-1">     
                        ...
                        %>
                            ...
                            <% end if %>  
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-4">  
                                        <label>묻힘</label>
                                        <input type="number" id="bury_door" class="form-control" name="fl" value="<%=bury_door%>" placeholder="묻힘" onkeypress="handleKeyPress(event, 'fl', 'fl')">
                                    </div>   
                                <% if GREEM_F_A = 2  or ( GREEM_F_A = 1 and ( GREEM_FIX_TYPE = 15 or GREEM_FIX_TYPE = 34 ) ) then 'GREEM_F_A=2(자동) , GREEM_F_A=1(수동)%>
                                    <div class="col-4"> 
                                        <label>지정오픈</label>
                        ...
                        <% else %>
                            <input class="form-control" type="number" value="<%=tw%>" onclick="location.replace('TNG1_B_suju4.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                            
                            <input class="form-control" type="number" value="<%=th%>" onclick="location.replace('TNG1_B_suju4.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                        
                            <% if GREEM_F_A = 2 then 'GREEM_F_A=2(자동) , GREEM_F_A=1(수동)%>
                            <input class="form-control" type="number" value="<%=ow%>" onclick="location.replace('TNG1_B_suju4.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                          
                            <% end if %>  
                            <input class="form-control" type="number" value="<%=oh%>" onclick="location.replace('TNG1_B_suju4.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                      
                            <input class="form-control" type="number" value="<%=fl%>" onclick="location.replace('TNG1_B_suju4.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                      
                            <% if GREEM_F_A = 2 then 'GREEM_F_A=2(자동) , GREEM_F_A=1(수동)%>
                            <input class="form-control" type="number" value="<%=ow_m%>" onclick="location.replace('TNG1_B_suju4.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                      
                            <% end if %>  
                        <% end if %>
                    </div>
                        <%
    ...
    <div class="second-row">    
        ...
        <div class="second-flex-grow"> <!-- 가운데 SVG 영역 -->  
            <!-- 두번째 줄 두 번째 칸 시작 -->
                <div class="canvas-container" id="svgCanvas" style="width: 100%; height: 100%; padding: 0px;" data-total-width="<%=sja_mwidth%>" data-total-height="<%=sja_mheight%>">
                    <div class="svg-container" style="width: 100%; height: 100%; padding: 0px;">
                        ...
                        <g id="viewport" transform="translate(0, 0) scale(1)">
                            ...
                            SQL = SQL & ", B.door_w, B.door_h , B.glass_w, B.glass_h, B.ysize,b.doortype,b.garo_sero,b.alength,a.ow,a.oh,a.fl,b.gls"
                            SQL = SQL & " FROM tk_framek A"
                        ...
                        If gls <> 0 Then
                            ' [유리 모드]
                            ' - 규격/방향 판단을 시스템이 이미 끝낸 상태라고 가정
                            ' - 계산된 결과값(alength, yblength)을 그대로 최종 치수에 반영
                            real_wi = alength    ' 최종 가로 = 시스템이 산출한 가로 길이
                            real_hi = yblength   ' 최종 세로 = 시스템이 산출한 세로 길이

                        Else
                            ' [자재 가로 세로 분류 모드]
                            ' - 사용자가 지정한 방향(garo_sero)에 따라 입력 치수의 축을 매핑
                            ' - 1이면 "가로 기준": ysize를 가로로, yblength를 세로로 사용
                            ' - 1이 아니면 "세로 기준/회전": yblength를 가로로, ysize를 세로로 사용(축 스왑)
                            If garo_sero = 1 Then
                                ' (가로 기준) 설치/도면 기준이 가로 방향일 때
                                '  - ysize: 가로로 놓일 변(폭/짧은 변 등)
                                '  - yblength: 세로로 놓일 변(높이/긴 변 등)
                                real_wi = ysize     ' 가로 ← 사용자 입력값 ysize를 가로축에 배치
                                real_hi = yblength   ' 세로 ← yblength(일반적으로 높이/긴 변)를 세로축에 배치
                            Else
                                ' (세로 기준/회전) 세로가 기준이 되거나, 가로/세로를 맞추기 위해 축을 뒤집는 경우
                                '  - yblength를 가로로 돌려 배치(가로가 더 길어야 하는 케이스 등)
                                '  - ysize는 세로로 배치
                                real_wi = yblength  ' 가로 ← 긴 변(yblength)을 가로축으로 전환
                                real_hi = ysize      ' 세로 ← 짧은 변(ysize)을 세로축으로 전환
                            End If
                        End If
                        ...
                        if fstype="2" then %>
                            ...
                            <rect 
                                ...
                                data-type="<%=rect_type%>" 
                                onclick="location.replace('TNG1_B_suju4.asp?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=fkidx%>&sjb_idx=<%=sjb_idx%>&fksidx=<%=fksidx%>');"
                            />
                        <% else %>
                            <rect 
                                x="<%=xi%>" 
                        ...
                        <% if request("new_open")="start" then %>
                            <rect 
                                ...
                                stroke="<%=stroke_text%>" 
                                stroke-width="1"
                                data-value="width=<%=real_wi%>,height=<%=real_hi%>,which=<%=WHICHI_AUTO%>"
                                data-type="<%=rect_type%>" 
                                onclick="location.replace('TNG1_B_suju4.asp?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=fkidx%>&sjb_idx=<%=sjb_idx%>&fksidx=<%=fksidx%>');"
                            />
                        <% end if %>
