...
If gubun="" then 
  ...
  SQL="Select A.blength, A.alength, A.xsize, A.ysize, A.xi, A.yi, B.tw, B.th, B.ow, B.oh, B.fl, A.groupcode "
  SQL=SQL&" From tk_framekSub A "
...
<head>
  ...
  <script>
    function lengthreset(){
      ...
      {
          location.href="rlength3.asp?gubun=lengthreset&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=rfkidx%>&sjb_idx=<%=rsjb_idx%>&fksidx=<%=rfksidx%>&sjb_type_no=<%=rsjb_type_no%>&greem_f_a=<%=rgreem_f_a%>"
      }
    }
    function del(){
      if (confirm("선택한 부속을 삭제 하시겠습니까?"))
      {
          location.href="rlength3.asp?gubun=del&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=rfkidx%>&sjb_idx=<%=rsjb_idx%>&fksidx=<%=rfksidx%>&sjb_type_no=<%=rsjb_type_no%>&greem_f_a=<%=rgreem_f_a%>"
      }
    }
    function framedel(){
      if (confirm("프레임의 모든 정보를 삭제 하시겠습니까?"))
      {
          location.href="rlength3.asp?gubun=framedel&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=rfkidx%>&sjb_idx=<%=rsjb_idx%>&fksidx=<%=rfksidx%>&sjb_type_no=<%=rsjb_type_no%>&greem_f_a=<%=rgreem_f_a%>"
      }
    }    
    ...
    function chg(whichi_fix, doorwhichi){
      ...
      if (result) {
        location.href="rlength3.asp?gubun=chg&mode=auto&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=rfkidx%>&sjb_idx=<%=rsjb_idx%>&fksidx=<%=rfksidx%>&sjb_type_no=<%=rsjb_type_no%>&greem_f_a=<%=rgreem_f_a%>&whichi_fix="+whichi_fix+"&doorwhichi="+doorwhichi;
      } else {
        location.href="rlength3.asp?gubun=chg&mode=manual&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=rfkidx%>&sjb_idx=<%=rsjb_idx%>&fksidx=<%=rfksidx%>&sjb_type_no=<%=rsjb_type_no%>&greem_f_a=<%=rgreem_f_a%>&whichi_fix="+whichi_fix+"&doorwhichi="+doorwhichi;
      }
    }
...
<div class="container">
  ...
  <form id="lengthForm" name="lengthForm" action="rlength3.asp" method="POST">
    ...
    <input type="hidden" name="yi" value="<%=yi%>">
...
<%  else %>
    <div class="mb-3 d-inline-flex">
      <label for="rlength" class="form-label mb-0">너비 (mm)://</label>
      <input type="text" id="rlength" name="blength" class="form-control" placeholder="예: 12,345" value="<%=blength%>" required>
      <button type="submit" class="btn btn-primary">길이적용</button>
    </div>
<% end if %>
    <div class="d-flex align-items-center gap-3 mb-3">
      ...
      <div class="form-check mb-0">
        <input class="form-check-input" type="radio" name="optionType" value="1" <% if zwhichi_fix_type<>"wb" Then Response.write "checked" end if %>>
        <label class="form-check-label" for="singleOption">단독적용</label>
      </div>

      <div class="form-check mb-0">
        <input class="form-check-input" type="radio" name="optionType" value="2"  <% if zwhichi_fix_type="wb" Then Response.write "checked" end if %>>
    ...
    <div  class="d-flex align-items-center gap-3 mb-3">
      ...
      <button type="button" class="btn btn-dark" onclick="">전체계산값적용</button>
    </div>
    <div  class="d-flex align-items-center gap-3 mb-3">
...
%>
      <div class="col-4 mb-2">
        <button type="button" class="btn btn-outline-success" onclick="chg('<%=whichi_fix%>','0');"><%=WHICHI_FIXname%></button>
      </div>
<%
  k = i mod 3
...
<% if groupcode="0"  then %>  
  <div class="row">
      <div class="col-6 mb-2">
        <button type="button" class="btn btn-outline-warning" onclick="chgmanual('0','1');">[롯트바]좌도어+우픽스</button>
      </div>
      <div class="col-6 mb-2">
        <button type="button" class="btn btn-outline-warning" onclick="chgmanual('0','2');">[롯트바]좌픽스+우도어</button>
      </div> 
    </div>
    <div class="row">
      <div class="col-6 mb-2">
        <button type="button" class="btn btn-outline-info" onclick="chgmanual('0','3');">[박스라인]좌도어+우픽스</button>
      </div>
      <div class="col-6 mb-2">
        <button type="button" class="btn btn-outline-info" onclick="chgmanual('0','4');">[박스라인]좌픽스+우도어</button>
      </div> 
    </div>
<% end if %>
    ...
    <div class="row">
      <h4 class="mb-2">상부남마 세로중간통바 추가</h4>
...
'===== 
  ...
  if cint(rfksidx)=cint(zfksidx) then '선택한 fksidx와 전달받은 키가 일치한다면 더이상 자동으로 길이를 업데이트 하지 못하도록 rstatus=1로 설정한다.
    ...
    SQL=SQL&" , bokgu_wi='"&zwi&"', bokgu_hi='"&zhi&"', bokgu_alength='"&alength&"', bokgu_blength='"&blength&"' "
...
'매개변수 다시받아 저장하기
  ralength=Request("alength")
  rblength=Request("blength")
...
Rs.close
  ...
  'response.end
  response.write "<script>location.replace('rlength3.asp?gubun=up1date&optionType="&roptionType&"&sjidx="&rsjidx&"&sjsidx="&rsjsidx&"&sjb_idx="&rsjb_idx&"&SJB_TYPE_NO="&rSJB_TYPE_NO&"&greem_f_a="&rgreem_f_a&"&fkidx="&rfkidx&"&fksidx="&fksidx&"&xi="&xi&"&alength="&alength&"&blength="&blength&"')</script>"
...
Elseif gubun="lengthreset" then 
  ...
  REsponse.write "<script>location.replace('rlength3.asp?sjidx="&rsjidx&"&sjsidx="&rsjsidx&"&fkidx="&rfkidx&"&sjb_idx="&rsjb_idx&"&fksidx="&rfksidx&"&sjb_type_no="&rsjb_type_no&"&greem_f_a="&rgreem_f_a&"');</script>"
