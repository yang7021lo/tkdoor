...
if rafksidx<>"" then 
  ...
  response.write "<script>location.replace('TNG1_B_suju_temp.asp?cidx="&rcidx&"&sjidx="&rsjidx&"&sjsidx="&rsjsidx&"')</script>"
end if
'===================
'바 추가 복제하기 끝
'개별 바의 길이 입력시작
...
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
  <script>
      ...
      if (message !== null && message.trim() !== "") {
        ...
        window.location.href = "TNG1_B_suju_temp.asp?part=pummoksub&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx="+sjb_idx+"&fknickname="+encodedMessage;
      }
    }
    function framedel(fkidx){
        if (confirm("프레임을 삭제 하시겠습니까?"))
        {
            location.href="TNG1_B_suju_temp.asp?part=framedel&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx="+fkidx;
        }
    }
    function bardel(fksidx){
        if (confirm("바를 삭제 하시겠습니까?"))
        {
            location.href="TNG1_B_suju_temp.asp?part=bardel&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx="+fksidx;
        }
    }
    function setstd(ni,fkidx){

        {
            location.href="TNG1_B_suju_temp.asp?part=setstd&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx="+fkidx+"&ni="+ni;
        }
    }
    function wresize(order){
        {
            location.href="TNG1_B_suju_temp.asp?part=wresize&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&order="+order;
        }
    }
    function hresize(order){
        {
            location.href="TNG1_B_suju_temp.asp?part=hresize&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&order="+order;
        }
    }
    function converge(direction){
        {
            location.href="TNG1_B_suju_temp.asp?part=converge&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&direction="+direction;
        }
    }
    
    function addglass(glasstype, wsize, hsize){
        {
            location.href="TNG1_B_suju_temp.asp?part=addglass&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&gubun=glass&glasstype="+glasstype+"&wsize="+wsize+"&hsize="+hsize;
        }
    }
 
    function addcovered(alocation){
        {
            location.href="TNG1_B_suju_temp.asp?part=addcovered&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&gubun=covered&alocation="+alocation;
        }
    }
    function wincopy(){
...
<div class="container-fluid">
    ...
    <div class="first-row">
        <div class="row px-3 w-100">
            ...
            <select name="pidx" class="form-control" id="pidx"  onchange="handleChange_dataFormOriginal(this)">
                <option value="0" <% if sja_qtyidx="" then %>selected<% end if %>>도장칼라 선택</option>
                    <%
                    sql = "SELECT pidx, pname FROM tk_paint WHERE pstatus = 1 "
                    ...
                    pname=Rs(1) 
                    %>
                                    <option value="<%=pidx%>" <% if Cint(pidx)=Cint(sja_pidx) then %> selected <% end if %> ><%=pname%></option>
                    <%
                    Rs.movenext
                    Loop
                    End if
                ...
                <div class="row">
                    <div class="col-9">
                        ...
                        <div class="input-group mb-1">     
                            ...
                            onclick="location.replace('TNG1_B_suju_temp.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>');" 
                ...
                <div class="mb-2">
                    ...
                    <div class="input-group mb-1">     
                        ...
                        %>
                            ...
                            <% end if %>  
                                <% if rfksidx<>"" and tw > 0 and th > 0 and oh > 0 and (rsjb_type_no = 6 Or rsjb_type_no = 7 Or rsjb_type_no = 11 Or rsjb_type_no = 12) Then %>
                                    <div class="col-4">
                                      <label>수치입력</label>
                                      <button class="btn btn-secondary  btn-small" type="button" Onclick="window.open('inspector_v2.asp?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&SJB_TYPE_NO=<%=rSJB_TYPE_NO%>&greem_f_a=<%=rgreem_f_a%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>','inspector','top=100 left=400 width=600 height=800');">접속</button>
                                    </div>
                                <% end if %>
                                </div>
                                <div class="row">
                        ...
                        <% else %>
                            <input class="form-control" type="number" value="<%=tw%>" onclick="location.replace('TNG1_B_suju_temp.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                            
                            <input class="form-control" type="number" value="<%=th%>" onclick="location.replace('TNG1_B_suju_temp.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                        
                            <% if GREEM_F_A = 2 then 'GREEM_F_A=2(자동) , GREEM_F_A=1(수동)%>
                            <input class="form-control" type="number" value="<%=ow%>" onclick="location.replace('TNG1_B_suju_temp.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                          
                            <% end if %>  
                            <input class="form-control" type="number" value="<%=oh%>" onclick="location.replace('TNG1_B_suju_temp.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                      
                            <input class="form-control" type="number" value="<%=fl%>" onclick="location.replace('TNG1_B_suju_temp.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                      
                            <% if GREEM_F_A = 2 then 'GREEM_F_A=2(자동) , GREEM_F_A=1(수동)%>
                            <input class="form-control" type="number" value="<%=ow_m%>" onclick="location.replace('TNG1_B_suju_temp.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>#<%=fkidx%>');"/>                      
                            <% end if %>  
                        <% end if %>
                    </div>
                        <%
                ...
                <div class="canvas-container" id="svgCanvas" style="width: 100%; height: 100%; padding: 0px;">
                    <div class="svg-container" style="width: 100%; height: 100%; padding: 0px;">
                        ...
                        if fstype="2" then %>
                            ...
                            onclick="location.replace('TNG1_B_suju_temp.asp?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=fkidx%>&sjb_idx=<%=sjb_idx%>&fksidx=<%=fksidx%>');" data-value="id=<%=fksidx%>;width=<%=yblength%>;height=<%=ysize%>;"/> 
                        <% else%>
                        ...
                        <% if request("new_open")="start" then %>
                            <rect x="<%=xi%>" y="<%=yi%>" width="<%=wi%>" height="<%=hi%>" fill="<%=fill_text%>" stroke="<%=stroke_text%>" stroke-width="1" 
                            onclick="location.replace('TNG1_B_suju_temp.asp?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=fkidx%>&sjb_idx=<%=sjb_idx%>&fksidx=<%=fksidx%>&new_open=go');" data-value="id=<%=fksidx%>;width=<%=yblength%>;height=<%=ysize%>;"/>
                        <% end if %>
                        <%
                            ' 중심 좌표 계산
                            centerX = xi + (wi / 2)
                            ...
                            If glassselect_fix="1" or glassselect_fix="2" then 
                              ' door_w * door_h
                              If IsNumeric(door_w) And IsNumeric(door_h) Then
                                  yblength = CStr(door_w) & "×" & CStr(door_h)&"/"&fksidx&"/A/"&whichi_fix&"/"
                              End If
                            Elseif glassselect_fix="3" or glassselect_fix="4" or glassselect_fix="5" or glassselect_fix="6" then 
                              ' glass_w * glass_h
                              If IsNumeric(glass_w) And IsNumeric(glass_h) Then
                                  yblength = CStr(glass_w) & "×" & CStr(glass_h)&"/"&fksidx&"/B/"&whichi_fix&"/"
                              End If
                            ElseIf glassselect_fix="0" then   '바라면
                              If IsNumeric(glass_w) And IsNumeric(glass_h) Then
                                  yblength = CStr(yblength) & "×" & CStr(ysize)&"/"&fksidx&"/C/"&whichi_fix&"/"
                              End If
                            End If
                        %>
                        <%
