...
ryaddr1 = encodesTR(uploadform("yaddr1"))
...
ryprepay = encodesTR(uploadform("yprepay"))  ' → "4,000원" 같은 문자열
    clean_prepay = Replace(ryprepay, ",", "") ' → "4000원"
    clean_prepay = Replace(clean_prepay, "원", "") ' → "4000"
    clean_prepay = Trim(clean_prepay)
    ' 숫자형으로 변환
    If IsNumeric(clean_prepay) Then
        clean_prepay = CLng(clean_prepay)  ' 금액은 큰 값일 수 있으니 CInt → CLng 권장
    Else
        clean_prepay = 0
    End If
...
if ryidx<>"" then 
  ...
  SQL=" Insert into tk_yongcha (sjidx, yname, ytel, yaddr, ydate, ymemo, ycarnum, ygisaname, ygisatel, ycostyn, yprepay, ystatus, ymidx, ywdate, ymeidx, ywedate ,yaddr1 ) "
  SQL=SQL&" Values ('"&rsjidx&"', '"&ryname&"', '"&rytel&"', '"&ryaddr&"', '"&rydate&"', '"&rymemo&"', '"&rycarnum&"', '"&rygisaname&"', '"&rygisatel&"' "
  SQL=SQL&" , '"&rycostyn&"', '"&clean_prepay&"', '1', '"&C_midx&"', getdate(), '"&C_midx&"', getdate() , '"&ryaddr1&"' )"
  Response.write (SQL)&"<br>"
  'response.end
  dbCon.execute (SQL)
