...
'tng_sjaSub/tk_framek 만들기 시작
  ...
  if not (Rs.EOF or Rs.BOF ) then
    ...
    SQL = SQL & " where b.fkidx='"&max_fkidx&"' and a.sjsidx='"&rsjsidx&"' "
    SQL = SQL & " and b.sunstatus=0 "
    SQL = SQL & "  ORDER BY b.xi DESC "
    'Response.write (SQL)&"<br><br>"
    Rs1.open Sql,Dbcon,1,1,1
    if not (Rs1.EOF or Rs1.BOF ) then
        maxxi = Rs1(0)
    ...
    If rSJB_TYPE_NO = 1 Or rSJB_TYPE_NO = 3 Then
        SQL = SQL & " AND a.sunstatus IN (0,1,5,6)"
    elseIf rSJB_TYPE_NO = 2 Or rSJB_TYPE_NO = 4 Then
        SQL = SQL & " AND a.sunstatus IN (0,1,2,3,4,5,6)"
    elseIf rSJB_TYPE_NO = 5 Then
        SQL = SQL & " AND a.sunstatus IN (0,5,6)"
    else
        SQL = SQL & " AND a.sunstatus IN (0,6)"
    end if
    '픽스 상부 오사이 추가 구문 끝
    Response.write (SQL)&"<br><br>"
    Rs1.open Sql,Dbcon
 ...
 //   const message = prompt("이 입면 도면을 기본으로 부속이 적용된 신규 부족적용 입면 도면 생성합니다. 입면도면의 이름을 입력하세요.");
  //      if (message !== null && message.trim() !== "") {
   //     const encodedMessage = encodeURIComponent(message.trim());
    //    window.location.href = "TNG1_B_choiceframe.asp?part=pummoksub&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_type_no=<%=rsjb_type_no%>&sjb_idx=<%=rsjb_idx%>&SearchWord=<%=SearchWord%>&cidx=<%=rcidx%>&fidx="+fidx+"&fknickname="+encodedMessage;
     //   }
 //   }
    <% if mode="quick" then %>
      function pummoksub(fidx){
      
            location.href="TNG1_B_choiceframe.asp?mode=quick&part=pummoksub&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_type_no=<%=rsjb_type_no%>&sjb_idx=<%=rsjb_idx%>&SearchWord=<%=SearchWord%>&fidx="+fidx;
    ...
    function pummoksub(fidx){
        ...
        {
            location.href="TNG1_B_choiceframe.asp?part=pummoksub&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_type_no=<%=rsjb_type_no%>&sjb_idx=<%=rsjb_idx%>&SearchWord=<%=SearchWord%>&fidx="+fidx;
        }
    }
    <% end if %>
  </script>
...
<form name="frmMainsub" action="TNG1_B_choiceframe.asp" method="POST">  
