...
g_bogang=Request("g_bogang")
g_busok=Request("g_busok")
rbaidx=Request("baidx")    '절곡키값
...
'response.write "bachannel : " & bachannel & "<br>"
...
if DDD="delete" then 
    ...
    response.write "<script>location.replace('TNG1_JULGOK_IN_SUB.asp?sjb_idx="&rsjb_idx&"&bfidx="&rbfidx&"');</script>"
end if
...
if part="binsert" then 
    ...
    SQL = "INSERT INTO tk_barasi (baname, bamidx, bawdate, bastatus, bachannel, bfidx, g_bogang, g_busok) " & _
    "VALUES ('" & baname & "', '" & c_midx & "', getdate(), '" & bastatus & "', '" & bachannel & "', '" & rbfidx & "', '" & g_bogang & "', '" & g_busok & "')"
    'Response.write (SQL)&"<br>"
    Dbcon.Execute (SQL)
...
if part="bupdate" then 
    SQL = "update tk_barasi set baname='" & baname & "' , bastatus='" & bastatus & "' , bachannel='" & bachannel & "' , "
    SQL = SQL & "g_bogang='" & g_bogang & "' , g_busok='" & g_busok & "' , bfidx='" & rbfidx & "' where baidx='" & rbaidx & "' "
    'Response.write (SQL)&"<br>"
    Dbcon.Execute (SQL)
    response.write "<script>location.replace('TNG1_JULGOK_IN_SUB.asp?sjb_idx="&rsjb_idx&"&bfidx="&rbfidx&"&baidx="&rbaidx&"&part=update');</script>"
end if 
...
'첫 입력 여부 확인 및 초기 변수 설정 시작
    ...
    If Not (Rs.bof or Rs.eof) Then '입력값이 하나라도 있다면
        ...
        x1=Clng(cx2)    '시작점 x좌표를 입력값으로 설정
        y1=Clng(cy2)    '시작점 y좌표를 입력값으로 설정
        x2=Clng(cx2)    '종점 x좌표를 입력값으로 설정
        y2=Clng(cy2)    '종점 y좌표를 입력값으로 설정
        pba=0   '최근 방향 0으로 초기화
...
' 방향에 대한 x2좌표 적용 끝
    ...
    if pba="0" then '첫 등록이라면
        ...
        accsize=Clng(paccsize)+Clng(pbassize)+Clng(idv) 
...
'==========================================
    ...
    If Not (Rs.bof or Rs.eof) Then 
        ...
        if Clng(min_x1) <= Clng(min_x2) then  
            sx1=min_x1
        else
            sx1=min_x2
        end if
        if Clng(max_x1) >= Clng(max_x2) then  
            sx2=max_x1
        else
            sx2=max_x2
        end if

        if Clng(min_y1) <= Clng(min_y2) then  
            sy1=min_y1
        else
            sy1=min_y2
        end if
        if Clng(max_y1) >= Clng(max_y2) then  
            sy2=max_y1
        else
            sy2=max_y2
        end if
...
'가로세로 최소좌표 최대좌표 구해서 업데이트 끝
    ...
    response.write "<script>location.replace('TNG1_JULGOK_IN_SUB.asp?sjb_idx="&rsjb_idx&"&baidx="&rbaidx&"&bfidx="&rbfidx&"');</script>"
