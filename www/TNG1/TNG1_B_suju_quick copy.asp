...
If Request("new_open") = "go" Then
    ...
    response.write "window.open('TNG1_B_suju2_pop_quick.asp?cidx=" & rcidx & _
        ...
        "&fksidx=" & rfksidx & "', '_blank', 'width=1000,height=600');"
    response.write "</script>"
...
response.write "<script>location.replace('TNG1_B_suju_quick.asp?cidx="&rcidx&"&sjidx="&rsjidx&"&sjsidx="&rsjsidx&"')</script>"
...
if mode="quick" then 
    ...
    if rsjsidx=""  then 
        ...
        sql="update tk_framek set  qtyidx='"&rqtyidx&"' , pidx='"&rpidx&"' , quan='"&rquan&"' "
        SQL=SQL&" Where sjsidx='"&rsjsidx&"' " 
        Response.write (SQL)&"<br>"
        'Response.end
        Dbcon.Execute (SQL)
...
<div class="container-fluid">
    ...
    <div class="first-row">
        <div class="row px-3 w-100">
            ...
            If IsNumeric(sja_pidx) And sja_pidx <> "" Then
                Select Case sja_qtyidx
                   ...
                   ' Case 0,5 'AL/블랙
                        'sja_pidx = 0
                    Case 1, 3, 7, 30, 37  '1=H/L 3=갈바 7=갈바/도장 30=AL/도장 37=H/L_1.5t
                        sja_pidx = sja_pidx 
                    Case else
                        sja_pidx = 0
        ...
        <button type="submit" id="hiddenSubmit1" style="display: none;"></button>
                        ...
                        <form id="dataForm" name="dataForm"  action="TNG1_B_suju_cal_quick.asp" method="POST" >   
                            <input type="hidden" name="sjidx" value="<%=rsjidx%>">
                            <input type="hidden" name="sjsidx" value="<%=rsjsidx%>">
                            <input type="hidden" name="sjb_idx" value="<%=rsjb_idx%>">
                                ...
                                <div class="row">
                                    ...
                                    <div class="input-group mb-2">  
                                        <select name="dooryn" class="form-control" id="dooryn"  onchange="handleChange_dataForm(this)">
                                            <option value="0" <% If Cint(zdooryn) = "0" Then Response.Write "selected" %>>도어나중</option>
                                    ...
                                    <div class="input-group mb-2" style="gap: 8px; align-items: center;">
                                        ...
                                        <input type="number" class="form-control" name="doorglass_t" value="<%=door_depth%>" onkeypress="handleKeyPress_dataForm(event, 'doorglass_t', 'doorglass_t')">
                                        <span class="input-group-text py-0 px-1 small">픽스유리</span>
                                        <input type="number" class="form-control" name="fixglass_t" value="<%=fix_depth%>" onkeypress="handleKeyPress_dataForm(event, 'fixglass_t', 'fixglass_t')">
                                    </div>   
                                    <div class="mb-2">
                                        <%
                                    ...
                                    <div class="canvas-container" id="svgCanvas" style="width: 100%; height: 100%; padding: 0px;">
                                        <div class="svg-container" style="width: 100%; height: 100%; padding: 0px;">
                                            ...
                                            <g id="viewport" transform="translate(0, 0) scale(1)">
                                                ...
                                                SQL = SQL & " WHERE A.sjidx = '" & rsjidx & "' AND A.sjsidx = '" & rsjsidx & "' "
                                                SQL = SQL & " and b.whichi_auto not in (2,24) " 'whichi_auto=2 박스커버 ,24=오사이
                                                SQL = SQL & " and b.xi <> 0 "
                                                'Response.write (SQL)&"<br>"
                                                'response.end
                                                Rs.open Sql,Dbcon
                                            ...
                                            <% else%>
                                                ...
                                                onclick="window.open(
                                                    'inspector_v3.asp?sjcidx=<%=rsjcidx%>&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=fkidx%>&sjb_idx=<%=sjb_idx%>&sjb_type_no=<%=sjb_type_no%>&fksidx=<%=fksidx%>&greem_f_a=<%=rgreem_f_a%>'
                                                    ,'inspector_v3','top=100 left=400 width=570 height=800');"
        ...
        <div class="col-2">
            ...
            -->    
                <!-- 두번째 줄 세 번째 칸 시작 -->
                    <div>
                        <% if rfkidx<>"" then %>
                            <button class="btn btn-success btn-small" type="button" Onclick="window.open('TNG1_B_doorhchg.asp?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&fkidx=<%=rfkidx%>&sjb_idx=<%=rsjb_idx%>&SJB_TYPE_NO=<%=rSJB_TYPE_NO%>&mode=quick','doorchg','top=100 left=400 width=500 height=400,scrollbars=yes,resizable=yes');">기타옵션</button>
                        <% end if %>
                        <!--
                        ...
                        -->
                            <button type="button"
                                    ...
                                    if (window.opener && !window.opener.closed) {
                                        window.opener.location.href = 'TNG1_B.asp?sjcidx=<%=rsjcidx%>&sjmidx=<%=rsjmidx%>&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&money_reset=0';
                                        window.close();
                                    } else {
                                        location.href = 'TNG1_B.asp?sjcidx=<%=rsjcidx%>&sjmidx=<%=rsjmidx%>&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&money_reset=0';
                                    }
                                    ">
                                견적완료
                            </button>
                    ...
                    <div class="menu-container">
                        ...
                        <div class="dropdown-content" id="custom-dropdown-3"> 
                            <div class="input-group mb-2" style="overflow-x: auto; white-space: nowrap;">
                                        ...
                                        SQL=SQL&" ,c.set_name_fix,c.set_name_auto ,a.sjb_idx, a.sjb_type_no "
                                        SQL=SQL&" from tk_framek A "
                                        ...
                                        Do while not Rs.EOF
                                            ...
                                            yylbn = yylbn & "<td style=background:#80ff80; >" & fksidx_last3 & "</td>"
                                            else
                                            yylbn = yylbn & "<td onclick=""location.replace('TNG1_B_suju_quick.asp?sjcidx=" & rsjcidx & "&sjidx=" & rsjidx & "&sjsidx=" & rsjsidx & "&fkidx=" & rfkidx & "&sjb_idx=" & sjb_idx & "&sjb_type_no=" & sjb_type_no & "&fksidx=" & fksidx & "');"" style='cursor:pointer;'>" & fksidx_last3 & "</td>"
                                            end if
