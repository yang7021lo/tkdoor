...
Response.Write "<script>location.replace('unittype_p.asp?SJB_IDX=" & rSJB_IDX & "');</script>"
...
Else
    ...
    '퍼센트를 업데이트 하면서 해당 단가 * 퍼센트 값으로 변경 하기'
    '먼저 기본가격이 저장되어 있는 SJB_IDX로 조회하여 값을 가져오기'
    sql = "SELECT SJB_IDX "
    ...
    ' 퍼센트가 업데이트가 되면 기본 가격을 가져온뒤 퍼센트 증가시킨뒤 가격 업데이트'
    sql = "SELECT * "
    sql = sql & " FROM tng_unitprice_t "
    sql = sql & " WHERE SJB_IDX = '" & SJB_IDX_P & "' "
    Rs2.open sql, Dbcon, 1, 1, 1
    ...
    If Not (Rs2.EOF Or Rs2.BOF) Then
        Do While Not Rs2.EOF
            ...
            sql_u = "UPDATE tng_unitprice_t "
            sql_u = sql_u & " SET price = '" & new_price & "' "
