...
if rafksidx<>"" then 
  ...
  response.write "<script>location.replace('tng1b_suju2.asp?cidx="&rcidx&"&sjidx="&rsjidx&"&sjsidx="&rsjsidx&"')</script>"
end if
'===================
'바 추가 복제하기 끝
...
'Response.write blength&"/"
  ...
  SQL="Select qtyco_idx From tk_qtyco Where QTYNo=10 and sheet_h >= '"&blength&"' order by sheet_h asc"
  'Response.write (SQL)&"<br><br>"
  Rs1.open Sql,Dbcon
  If Not (Rs1.bof or Rs1.eof) Then 
    qtyco_idx=Rs1(0)
...
sprice = unitprice * pcent '할증적용 가격
  ...
  SQL=SQL&" Set blength='"&rblength&"', unitprice='"&unitprice&"', pcent='"&pcent&"', sprice='"&sprice&"' "
...
if rsjsidx<>"" then 
  ...
  Rs.open Sql,Dbcon
    rsjsidx=Rs(0)
  Rs.Close
  response.write "<script>location.replace('tng1b_suju2.asp?cidx="&rcidx&"&sjidx="&rsjidx&"&sjb_type_no="&rsjb_type_no&"&sjsidx="&rsjsidx&"')</script>"
End if
'===================
'품목정보가 없을 경우 생성 끝
...
<head>
  ...
  <!--#include virtual="/tng1/tng1b_suju2.css"-->
  ...
  <script>
    ...
    function pummoksub(sjb_idx) {
      ...
      if (message !== null && message.trim() !== "") {
        ...
        window.location.href = "tng1b_suju2.asp?part=pummoksub&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx="+sjb_idx+"&fknickname="+encodedMessage;
      }
    }
    function framedel(fkidx){
        if (confirm("프레임을 삭제 하시겠습니까?"))
        {
            location.href="tng1b_suju2.asp?part=framedel&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx="+fkidx;
        }
    }
    function bardel(fksidx){
        if (confirm("바를 삭제 하시겠습니까?"))
        {
            location.href="tng1b_suju2.asp?part=bardel&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx="+fksidx;
        }
    }
    ...
    function setstd(ni,fkidx){
        ...
        {
            location.href="tng1b_suju2.asp?part=setstd&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx="+fkidx+"&ni="+ni;
        }
    }
    function wresize(order){
        {
            location.href="tng1b_suju2.asp?part=wresize&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&order="+order;
        }
    }
    function hresize(order){
        {
            location.href="tng1b_suju2.asp?part=hresize&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&order="+order;
        }
    }
    function converge(direction){
        {
            location.href="tng1b_suju2.asp?part=converge&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&direction="+direction;
        }
    }
    function chgbarasif(bfidx){
        if (confirm("바의 품목을 변경 하시겠습니까?"))
        {
            location.href="tng1b_suju2.asp?part=chgbarasif&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&bfidx="+bfidx;
        }
    }
    function addglass(glasstype, wsize, hsize){
        {
            location.href="tng1b_suju2.asp?part=addglass&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&gubun=glass&glasstype="+glasstype+"&wsize="+wsize+"&hsize="+hsize;
        }
    }
 
    function addcovered(alocation){
        {
            location.href="tng1b_suju2.asp?part=addcovered&sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>&gubun=covered&alocation="+alocation;
        }
    }
    function wincopy(){
...
<body>
<form id="dataForm" name="dataForm" action="tng1b_suju2.asp" method="POST" >   
...
%>
            ...
            <li><a class="dropdown-item" onclick="window.open('choiceframe.asp?sjidx=<%=rsjidx%>&sjsidx=<%=rsjsidx%>&sjb_idx=<%=sjb_idx%>&sjb_type_no=<%=sjb_type_no%>&greem_f_a=<%=sjb_fa%>','choice','top=0 left=0 width=800, height=700');"><%=sjb_type_name%>&nbsp;<%=SJB_barlist%></a></li>
<%
    Rs.movenext
    Loop
    End if
...
%>
        <div class="input-group mb-1">     
          <input type="text" class="form-control" value="<%=maintext%><%=fname%>_<%=setstd%>" onclick="location.replace('tng1b_suju2.asp?sjsidx=<%=rsjsidx%>&sjidx=<%=rsjidx%>&sjb_idx=<%=sjb_idx%>&fkidx=<%=fkidx%>');" <% if Cint(fkidx)=Cint(rfkidx) then %>style="background-color: #D3D3D3;"<% end if %>>  
...
qtyname=Rs(1)
%>
                <option value="<%=qtyidx%>" <% if Cint(qtyidx)=Cint(rqtyidx) then %>selected<% end if %>><%=qtyname%>/<%=qtyidx%>/<%=rqtyidx%></option>
<%
Rs.movenext
Loop
End if
...
%>
          ...
          <div class="input-group mb-2">
            ...
            <input type="number" class="form-control" name="mheight" id="mheight" placeholder="검세로"  value="<%=rmheight%>" onkeypress="handleKeyPress(event, 'mheight', 'mheight')">
          </div>
<%
...
SQL="select count(*) from tk_framek where sjidx='"&rsjidx&"'"
Rs.open Sql,Dbcon
  cntni=Rs(0)
Rs.close
...
<div class="container mt-2" style="padding:2px;" >
  <div class="button-grid">
    <button type="button" class="btn btn-secandary" onclick="location.replace('tng1b_suju2.asp?gubun=glass&sjidx=<%=rsjidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>');">유리추가</button>
<% if rfksidx<>"" then %>
    <button type="button" class="btn btn-secandary" onclick="location.replace('tng1b_suju2.asp?gubun=covered&sjidx=<%=rsjidx%>&sjb_idx=<%=rsjb_idx%>&fkidx=<%=rfkidx%>&fksidx=<%=rfksidx%>');">묻힘추가</button>
<% else %>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 1) 사각형 파싱 (el, idx 포함)
        const rectEls = Array.from(viewport.querySelectorAll('rect'));
        const rectData = rectEls.map((r, i) => {
            ...
            return { x, y, w, h, vertices, el: r, idx: i + 1 };
        });

        // 1.1) 각 도형의 중앙에 idx 라벨 표시
        rectData.forEach(item => {
            const centerX = item.x + item.w / 2;
            const centerY = item.y + item.h / 2;

            const idxLabel = document.createElementNS(SVG_NS, 'text');
            idxLabel.setAttribute('x', centerX);
            idxLabel.setAttribute('y', centerY);
            idxLabel.setAttribute('fill', '#000');
            idxLabel.setAttribute('font-size', '14px');
            idxLabel.setAttribute('text-anchor', 'middle');         // 가로 중앙 정렬
            idxLabel.setAttribute('dominant-baseline', 'middle');    // 세로 중앙 정렬
            idxLabel.textContent = item.idx;
            viewport.appendChild(idxLabel);
        });
        ...
        rectData.forEach(item => {
            item.vertices.forEach(v => {
                ...
                label.setAttribute('x', v.x + 5);
                label.setAttribute('y', v.y - 5);
        ...
        // 3) 같은 x값 그룹별 y좌표 수집
        const offset = 10;
        const xs = [...new Set(rectData.flatMap(r => r.vertices.map(v => v.x)))].sort((a, b) => a - b);

        // 각 x별 로컬 MaxYv + offset 계산
        const lowers = xs.map(xv => {
            const ys = rectData
                .flatMap(r => r.vertices.filter(v => v.x === xv).map(v => v.y));
            return Math.max(...ys) + offset;
        });

        // 글로벌 최하단: 모든 lowers 중 가장 큰 값
        const globalLowerY = Math.max(...lowers);

        // 4) 같은 x값별 수직선 그리기 (minY → globalLowerY) + 엔드포인트 표시
        xs.forEach(xv => {
            // 해당 x에서의 모든 y 좌표
            const ysAtX = rectData
                .flatMap(r => r.vertices.filter(v => v.x === xv).map(v => v.y));
            const minYv = Math.min(...ysAtX);

            // 4.1) 수직선 생성
            const line = document.createElementNS(SVG_NS, 'line');
            line.setAttribute('x1', xv);
            line.setAttribute('y1', minYv);
            line.setAttribute('x2', xv);
            line.setAttribute('y2', globalLowerY);
            line.setAttribute('stroke', '#000');
            line.setAttribute('stroke-width', '1');
            // id 부여 (예: vline-150)
            line.setAttribute('id', `vline-${xv}`);
            viewport.appendChild(line);

            // 4.2) 끝점 차등 마커 (원) 추가
            const endMarker = document.createElementNS(SVG_NS, 'circle');
            endMarker.setAttribute('cx', xv);
            endMarker.setAttribute('cy', globalLowerY);
            endMarker.setAttribute('r', 4);
            endMarker.setAttribute('fill', 'red');
            // id 부여 (예: endpoint-150)
            endMarker.setAttribute('id', `endpoint-${xv}`);
            viewport.appendChild(endMarker);

        });

        // … (4번 로직까지 완료된 상태)

        // 5) endMarker 간 선분 연결 + 매칭 도형 표시
        (() => {
            // 5.1) endMarker만 골라 x순 정렬
            const endpoints = Array.from(
                viewport.querySelectorAll('circle[id^="endpoint-"]')
            )
                .map(c => ({ x: +c.getAttribute('cx'), y: +c.getAttribute('cy') }))
                .sort((a, b) => a.x - b.x);

            // 5.2) 인접한 두 점 연결
            endpoints.slice(0, -1).forEach((p, i) => {
                const q = endpoints[i + 1];
                const segLength = q.x - p.x;            // px 단위 길이
                ...
                seg.setAttribute('id', `seg-${Math.round(p.x)}-${Math.round(q.x)}`);
                seg.setAttribute('stroke-dasharray', `${(i + 1) * 4},${(i + 1) * 2}`);
                viewport.appendChild(seg);

                // ——— seg 길이 라벨 ———
                const lenLbl = document.createElementNS(SVG_NS, 'text');
                lenLbl.setAttribute('x', midX);
                lenLbl.setAttribute('y', midY + 15);
                lenLbl.setAttribute('fill', '#000');
                lenLbl.setAttribute('font-size', '8px');
                lenLbl.textContent = `${segLength}px`;
                viewport.appendChild(lenLbl);

                // ——— 1) segLength와 똑같고, x가 p.x인 도형만 필터링
                const matches = rectData.filter(r =>
                    r.w === segLength    // 너비 일치
                    && r.x === p.x       // seg 왼쪽 끝(p.x) 바로 위
                );

                if (matches.length > 0) {
                    // ——— 2) 해당 도형 강조 (파란 테두리)
                    matches.forEach(r => {
                        r.el.setAttribute('stroke', 'blue');
                        r.el.setAttribute('stroke-width', '3');
                    });
                    ...
                    // ——— 4) seg 위에 어떤 도형인지 라벨
                    const matchLbl = document.createElementNS(SVG_NS, 'text');
                    matchLbl.setAttribute('x', midX);
                    matchLbl.setAttribute('y', midY + 30);
                    matchLbl.setAttribute('fill', 'blue');
                    matchLbl.setAttribute('font-size', '6px');
                    matchLbl.setAttribute('text-anchor', 'middle');
                    matchLbl.textContent =
                        `Match Rect: ${matches.map(r => `#${r.idx}`).join(', ')}`;
                    viewport.appendChild(matchLbl);
                }
            });
        })();
    });
