...
<style>
  :root{
    --wmm:100; --hmm:25; --pad:2; --radius:2.5mm; --line:0.2mm;
  ...
  @page label { size: 100mm 25mm; margin: 0; }
  ...
  <table aria-label="제품 생산관리 표">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td colspan="4" data-max="3" data-min="2.2"><%=cgaddr%> <%=loc1%> <%=loc2%></td>
        <!--
        <th class="v-vert">유리</th>
        <td colspan="2" data-max="3" data-min="2.2"><%=doorglass_w%> × <%=doorglass_h%></td>
        <th class="v-vert">두께</th>
        <td colspan="1" data-max="3" data-min="2.2"><%=fixglass_t%></td>
        -->
      </tr>
    </tbody>
  </table>
...
<script>
/* 글자수에 비례해 1차 축소 → 2차 이진탐색으로 최대치 보정 (줄넘김 없이 … 처리 유지) */
(function(){
  // ===== 전역 설정(일괄 적용) =====
  ...
  function fitOne(td){
    ...
    // 2) 보정 — 여유가 있으면 키우고, 넘치면 줄여서 "가능한 크게"
    if (fits(td)){
      // 위로 최대까지 탐색
      ...
      // 아래로 줄이며 맞추기 (그래도 안 맞으면 최소 폰트 + ellipsis 유지)
