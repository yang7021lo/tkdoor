...
<%
    ...
    gubun=Request("gubun")
    pidx=Request("pidx") 
    oes="select pname, pstatus from tk_paint where pidx='"&pidx&"'  "
    response.write (oes)
    Rs.open oes,dbCon
...
<!-- 내용 입력 시작 -->  
<form name="frmMain" action="paintmgntudtdb.asp" method="post">
...
<!-- input 형식 시작--> 
        ...
        <div class="input-group mb-3">
            ...
            <input class="form-check-input" type="radio" name="pstatus" value="1" <% if pstatus="1" then %>checked<% end if %> >
            <label class="form-check-label" for="flexRadioDefault1">
                사용함
            </label>
            </div>&nbsp;&nbsp;
            <div class="form-check">
            <input class="form-check-input" type="radio" name="pstatus"  value="0" <% if pstatus="0" then %>checked<% end if %>>
