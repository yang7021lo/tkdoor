...
if gubun<>"" then 
    ...
    SQL=" Insert into tk_goods (gotype, gocode, gocword, goname, gomidx, gowdate,gostatus) values  ('1','"&gocode&"','"&gocword&"','"&goname&"', '"&C_midx&"', getdate()), 1 "
    'response.write (SQL)
    'response.end
    DbCon.Execute (SQL)
        Response.write "<script>window.parent.location.replace('pummok_door.asp');</script>"
        response.end
    end if
else
%>
...
%> 
                                        <tr <% if Cint(goidx)=Cint(rgoidx) then %>class="bg-warning"<% end if %> >
                                            ...
                                            <td><%=gocode%></td>
                                            <td><%=gocword%></td>
                                            <td><a onclick="window.parent.location.replace('pummok_door.asp?rgoidx=<%=goidx%>');"><%=goname%></a></td>
<!--
