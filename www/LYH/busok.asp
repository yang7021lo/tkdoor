...
If gubun="insert" and rgoidx<>"" and rsidx<>"" and rbuidx<>"" Then 
    ...
    SQL="Insert into tk_material (sidx, buidx, smmidx, smwdate ) "
    SQL=SQL&" Values  ('"&rsidx&"', '"&rbuidx&"', '"&C_midx&"', getdate() ) "
    Dbcon.Execute (SQL)
    Response.write "<script>window.parent.location.replace('mes3.asp?rgoidx="&rgoidx&"&rsidx="&rsidx&"&rbuidx="&rbuidx&"');</script>"
    response.end
else
%>
<!DOCTYPE html>
...
SQL=" Select A.BUIDX, A.buname, A.atype, A.bustatus, A.bumidx, Convert(Varchar(10),A.buwdate,121), B.mname , Convert(Varchar(10),A.buewdate,121), C.mname "
...
%> 
                                        <tr>
                                            <td><%=yyy%></td>
                                            <td><a href="busok.asp?gubun=insert&rgoidx=<%=rgoidx%>&rsidx=<%=rsidx%>&rbuidx=<%=buidx%>"><%=buname%></a></td>
                                        </tr>
<%
