...
<select id="searchMold" class="form-select form-select-sm">
  ...
  <% RsMold.MoveNext : Loop %>
      ...
      <div class="col-md-3 text-end">
        <button class="btn btn-sm btn-primary me-1" onclick="filterMaterialTable()">검색</button>
        <button class="btn btn-sm btn-outline-secondary" onclick="resetMaterialFilter()">초기화</button>
      </div>
...
function filterMaterialTable(){
  ...
  document.querySelectorAll("#materialTable tbody tr").forEach(tr=>{
    ...
    // 입력 행 / 안내 행은 항상 표시
    if(
      tr.classList.contains("material-insert-row") ||
    ...
    /* 원자재명 */
    if(name){
      const matName = (tds[1]?.innerText || "").toLowerCase();
      if(!matName.includes(name)) show = false;
    }

    /* 길이 */
    if(length){
      if(tr.dataset.lengthId !== length) show = false;
    }

    /* 표면처리 */
    if(surface){
      if(tr.dataset.surfaceId !== surface) show = false;
    }

    /* 금형 */
    if(mold){
      if(tr.dataset.moldId !== mold) show = false;
    }
