...
sql = "SELECT m.material_id, m.master_id, " & _
      ...
      "l.bom_length, s.surface_name, mo.mold_name, mo.mold_no, " & _
...
RsMold.Open "SELECT mold_id, mold_name, mold_no FROM bom2_mold WHERE master_id=" & master_id & " and is_active=1 ORDER BY mold_name", Dbcon
...
<div class="card mb-3">
  <div class="card-body p-3">
    <div class="row g-2 align-items-end">
      ...
      <!-- 원자재명 -->
      <div class="col-md-3">
        <label class="form-label mb-1">원자재명</label>
        <input type="text"
      ...
      <!-- 길이 -->
      <div class="col-md-2">
        <label class="form-label mb-1">길이</label>
<select id="searchLength" class="form-select form-select-sm">
  <option value="">전체</option>
  <% RsLen.MoveFirst
     Do While Not RsLen.EOF %>
    <option value="<%=RsLen("length_id")%>">
      <%=RsLen("bom_length")%>
    </option>
  <% RsLen.MoveNext : Loop %>
</select>
      ...
      <!-- 표면처리 -->
      <div class="col-md-2">
        <label class="form-label mb-1">표면처리</label>
<select id="searchSurface" class="form-select form-select-sm">
  <option value="">전체</option>
  <% RsSurf.MoveFirst
     Do While Not RsSurf.EOF %>
    <option value="<%=RsSurf("surface_id")%>">
      <%=RsSurf("surface_name")%>
    </option>
  <% RsSurf.MoveNext : Loop %>
      ...
      <!-- 금형 -->
      <div class="col-md-2">
        <label class="form-label mb-1">금형명</label>
<select id="searchMold" class="form-select form-select-sm">
  <option value="">전체</option>
  <% RsMold.MoveFirst
     Do While Not RsMold.EOF %>
    <option value="<%=RsMold("mold_id")%>">
      <%=RsMold("mold_no")%>
    </option>
...
<button type="button"
        ...
        onclick="filterMaterialTable()">검색</button>

<button type="button"
        class="btn btn-sm btn-outline-secondary"
        onclick="resetMaterialFilter()">초기화</button>
      </div>
...
<% Do While Not RsMold.EOF %>
<option value="<%=RsMold("mold_id")%>"><%=RsMold("mold_no")%></option>
<% RsMold.MoveNext : Loop %>
</select>
...
function filterMaterialTable(){
  ...
  const name    = document.getElementById("searchMaterialName").value.trim().toLowerCase();
  const length  = document.getElementById("searchLength").value;
  const surface = document.getElementById("searchSurface").value;
  const mold    = document.getElementById("searchMold").value;

  document.querySelectorAll("#materialTable tbody tr").forEach(tr=>{

    // 입력 행은 항상 표시
    if(tr.classList.contains("material-insert-row")){
      tr.style.display = "";
      return;
    }
    ...
    if(name !== ""){
      const matName =
        tr.querySelectorAll("td")[1]?.innerText.toLowerCase() || "";
    ...
    ================================ */
    if(show && surface !== ""){
      if((tr.dataset.surfaceId || "") !== surface){
        show = false;
      }
    }
...
function resetMaterialFilter(){
  ...
  document.getElementById("searchLength").value = "";
  document.getElementById("searchSurface").value = "";
  document.getElementById("searchMold").value = "";
...
function bindMaterialSearchEnter(){
  const targets = [
    document.getElementById("searchMaterialName"),
    document.getElementById("searchLength"),
    document.getElementById("searchSurface"),
    document.getElementById("searchMold")
  ];
...
// DOM 로드 후 실행
document.addEventListener("DOMContentLoaded", bindMaterialSearchEnter);
