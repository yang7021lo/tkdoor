...
function saveSubValueRow(){
    ...
    .then(res=>{
        if(res.trim() === "OK"){
            appendSubValueRow(masterId, values);
...
function appendSubValueRow(masterId, values){
    ...
    /* MASTER */
    html += `<td>${getMasterNameById(masterId)}</td>`;
    ...
    document
        ...
        .forEach(th=>{
            ...
            const found = values.find(v=>v.titleSubId === subId);

            html += `<td class="sub-value">${found ? found.value : ""}</td>`;
        });
