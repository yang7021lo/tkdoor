...
<tr data-material-id="<%=RsMat("material_id")%>">
  ...
  <span class="view-value"
    ...
    <%=Server.HTMLEncode(RsMat("material_name"))%>
  </span>
  </td>
...
<td>
  <span class="view-value"
    ...
    <% Else %>
      data-title-id="<%=RsC("list_title_id")%>"
    <% End If %>>
    <!-- 값은 JS 또는 JOIN으로 채워질 영역 -->
  </span>
</td>
<%
    RsC.MoveNext
...
function collectMaterialRow(tr){
  const data = {
    ...
    material_name: tr.querySelector("[data-field='material_name']").value,
    titles: [],
    subs: []
  };
  ...
  // sub (select + show)
  tr.querySelectorAll("[data-role='select-sub']").forEach(sel=>{
    const opt = sel.options[sel.selectedIndex];
    if(!opt || !opt.dataset.rowId) return;

    data.subs.push({
      title_sub_id: sel.dataset.titleSubId,
      row_id: opt.dataset.rowId
    });
  });
...
function saveMaterialRow(btn){
  ...
  form.method = "POST";
  form.action = "bom3_material_save.asp";

  // master_id
  form.appendChild(input("master_id", "<%=master_id%>"));

  // material_id (기존 row)
  if(tr.dataset.materialId){
    form.appendChild(input("material_id", tr.dataset.materialId));
  }

  // material_name
  const nameInput =
    tr.querySelector("[data-field='material_name']") ||
    tr.querySelector("td:nth-child(2) input");

  form.appendChild(input("material_name", nameInput.value));

  // 일반 title
  tr.querySelectorAll("[data-title-id]").forEach(el=>{
    form.appendChild(
      input("title_" + el.dataset.titleId, el.value)
    );
  });
  ...
  tr.querySelectorAll("select[data-role='select-sub']").forEach(sel=>{
    ...
    if(opt && opt.value){
      form.appendChild(
        input("subsel_" + sel.dataset.titleSubId, opt.value)
      );
    }
  });
