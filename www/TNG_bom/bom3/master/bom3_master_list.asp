...
sql = "SELECT m.master_id, m.item_no, m.item_name, m.is_active, m.cdate, o.origin_type_no, o.origin_name " & _
      "FROM bom3_master m " & _
      "JOIN bom3_origin_type o " & _
...
<tr data-id="<%=Rs("master_id")%>"
    >
    <td><%=Rs("item_no")%></td>
<td class="master-name" style="cursor:pointer;">
    <span onclick="openMaterialPopup(<%=Rs("master_id")%>, <%=Rs("is_active")%>)">
        <%=Rs("item_name")%>
    </span>

    <i class="bi bi-box-seam material-icon"
       title="자재(Material) 관리"
       onclick="event.stopPropagation(); openMaterialPopup(<%=Rs("master_id")%>, <%=Rs("is_active")%>)"></i>
</td>
...
<button class="btn btn-sm btn-danger"
  onclick="location.href='common/bom3_master_deactivate.asp?master_id=<%=Rs("master_id")%>'">
  중지
</button>
<% End If %>
    </td>
