...
<html lang="ko">
  <head>
    <meta charset="utf-8">
    <title>태광 AL 발주 출력 - Paletto Agency</title>
...
<link href="/documents/insideOrder/assets/index.css" rel="stylesheet" />
...
If Not (Rs.BOF Or Rs.EOF) Then
  ...
  %>
  <script>
    const base = '?sjidx=<%=sjidxSql%>&sjsidx=<%=sjsidxSql%>';
    window.open('/TNG1/TNG1_B_baljuST.asp' + base, '_blank');
    window.open('/TNG1/TNG1_B_baljuAL.asp' + base, '_blank');
    //document.write('<h3 style="font-family:맑은고딕;">스테인리스 제품으로 자동 출력되었습니다.</h3>');
  </script>
  <%
  Response.End
End If
...
' -------------------------------
' 자동도어 발주서 LOOP (Server.Execute)
'  - 쿼리스트링 금지, Session으로 전달
' -------------------------------
If autoCount > 0 Then
  For i = 0 To autoCount - 1
    Session("insideOrder.sjidx")  = sjidx
Session("insideOrder.sjsidx") = sjsidx
Session("insideOrder.fkidx")  = arrAuto(i)

Server.Execute "/documents/insideOrder/auto/index.asp"

    Response.Write "<hr>"  ' 구분선 원하면 사용
    ' Response.Flush          ' 스트리밍 원하면 사용
  Next
End If
...
If manualCount <> 0 Then
    Session("insideOrder.sjidx")  = sjidx
    Session("insideOrder.sjsidx") = sjsidx
    Session("insideOrder.fkidx")  = CLng(arrManual(0))

    Server.Execute "/documents/insideOrder/manual/index.asp"
    ' Response.Write "<hr>"
End If
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  <!-- 플로팅 인쇄 버튼 -->
<button type="button"
        class="btn btn-dark btn-lg rounded-circle shadow position-fixed print-fab"
        aria-label="인쇄"
        title="인쇄">
  🖨️
</button>
...
<style>
  .print-fab{
    right:16px; bottom:16px; z-index:1050;
...
<script>
  document.addEventListener('click', function(e){
    if(e.target.closest('.print-fab')) window.print();
  });
</script>
