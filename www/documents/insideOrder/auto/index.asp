...
' 섹션 색상 칠하는 함수
Dim ft, bgClass, frame_type_name, font_color
ft = Rs("frame_type")
Select Case ft
  Case "3","7"       ' 초록 배경
    bgClass = "bg-green"
...
%>
 ...
 <section class="no-break <%= bgClass %>">
    <table class="table-sm table-bordered table-fixed mb-2">
      ...
      <tbody>
        <tr>
          <td class="kv"><span class="k <%= font_color %>">발주처</span><span class="v"><%=Rs("c_name")%></span></td>
          <td class="kv">
            <span class="k <%= font_color %>">수주일자</span>
            <span class="v">
                <%= Server.HTMLEncode(Rs("sa_sjdate")) %>
                (<%= KDowFromYMD(Rs("sa_sjdate")) %>)
            </span>
          </td>  
          <td class="kv"><span class="k <%= font_color %>">수주번호</span><span class="v"><%=Rs("sa_sjnum")%>__No<%=rs("sunno")%></span></td>
        </tr>
        <tr>
          <td class="kv"><span class="k <%= font_color %>">현장명</span><span class="v"><%=Rs("sa_addr")%></span></td>
          <td class="kv">
            <span class="k <%= font_color %>">출고일자</span>
            <span class="v">
          ...
          <td class="kv">
          <% if Rs("qtyidx") <> 5 then %>
            <span class="k <%= font_color %>">도장출고일자</span>
            <span class="v">
        ...
        <tr>
          <td class="kv"><span class="k <%= font_color %>">출고방식</span><span class="v"><%=cgtype%></span></td>
          <td class="kv"><span class="k <%= font_color %>">수량</span><span class="v"><%=Rs("sas_quan")%>개</span></td>
          <td class="kv"><span class="k <%= font_color %>">재질/도어</span><span class="v"><%=Rs("q_name")%> · <%=Rs("fk_dooryn")%></span></td>
        </tr>
        <tr>
          <td class="kv"><span class="k <%= font_color %>">검측</span><span class="v"><%=Rs("fk_tw")%> × <%=Rs("fk_th")%>_묶음<%=fksCount%>개</span></td>
          <td class="kv"><span class="k <%= font_color %>">오픈</span><span class="v"><%=Rs("fk_ow")%> × <%=Rs("fk_oh")%></span></td>
          <td class="kv"><span class="k <%= font_color %>">색상</span><span class="v"><%=Rs("p_name")%></span></td>
        </tr>
        <tr>
...
<style>
/* 배경 구분 */
.bg-door12  { background:#eef7ff !important; }  /* WHICHI_AUTO 1,2 */
.bg-8-9-24  { background:#fff3e6 !important; }  /* WHICHI_AUTO 8,9,24 */
</style>
        ...
        <section class="section">
          <table class="table table-bordered table-fixed items mb-0">
            <colgroup>
              <col class="name"><col class="len"><col class="qty">
              <col class="name"><col class="len"><col class="qty">
            </colgroup>
            <thead>
              <tr class="text-center">
                <th>자재명</th><th>절단길이</th><th>수량</th>
                <th>자재명</th><th>절단길이</th><th>수량</th>
              </tr>
            </thead>
            <tbody>
<%
' ===== 유틸: 이름/길이 정규화(숫자 변환 없음) =====
Function NormName(s)
  Dim t : t = Trim("" & s)
...
Dim sql2, cmd2, rs2
        sql2 = ""
        sql2 = sql2 & "SELECT A.fkidx, B.blength, C.set_name_AUTO, " _
                  & "ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) AS WHICHI_AUTO, " _
                  & "W.WHICHI_AUTOname AS zip_name_AUTO " _
                  & "FROM tk_framek A WITH (NOLOCK) " _
                  & "LEFT JOIN tk_framekSub B WITH (NOLOCK) ON A.fkidx = B.fkidx " _
                  & "LEFT JOIN tk_barasiF C WITH (NOLOCK) ON B.bfidx = C.bfidx " _
                  & "LEFT JOIN tng_whichitype W ON B.WHICHI_AUTO = W.WHICHI_AUTO " _
                  & "WHERE A.sjsidx = ? AND ISNULL(B.gls,0) = 0 " _
                  & "  AND A.fkidx = ? " _
                  & "ORDER BY " _

      ' ---------------------------------------------------------
      ' ① 네가 추가하고 싶어한 WHICHI_AUTO 커스텀 정렬
      ' ---------------------------------------------------------
       sql2 = sql2 _
          & "  CASE ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) " _
          & "        WHEN 1 THEN 1 " _
          & "        WHEN 2 THEN 2 " _
          & "        WHEN 3 THEN 3 " _
          & "        WHEN 6 THEN 4 " _
          ...
          & "  END, " _
      ' ---------------------------------------------------------
      ' ② 기존 ORDER BY 조건 그대로 적용
      ' ---------------------------------------------------------
      sql2 = sql2 _
        & "  CASE " _
        & "    WHEN ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) IN (6,7) THEN 2 " _
        & "    WHEN ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) IN (8,9,24) THEN 3 " _
        & "    WHEN ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) BETWEEN 0 AND 10 THEN 1 " _
        & "    ELSE 4 " _
        & "  END, " _
        & "  CASE " _
        & "    WHEN ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) IN (6,7) THEN " _
        & "      CASE ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) WHEN 6 THEN 1 WHEN 7 THEN 2 END " _
        & "    WHEN ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) IN (8,9,24) THEN " _
        & "      CASE ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) WHEN 8 THEN 1 WHEN 9 THEN 2 WHEN 24 THEN 3 END " _
        & "    ELSE ISNULL(W.WHICHI_AUTO, B.WHICHI_AUTO) " _
        & "  END, " _
          & "  C.set_name_AUTO, B.blength"
...
' adVarChar = 200
cmd2.Parameters.Append cmd2.CreateParameter("@p1", 200, 1, 20, ("" & sjsidx))
cmd2.Parameters.Append cmd2.CreateParameter("@p2", 200, 1, 20, ("" & rfkidx))

'response.write "sql : "&sjsidx& "<br>"

Set rs2 = cmd2.Execute

' 그룹
Dim dict, orderKeys(), ocnt
Set dict = Server.CreateObject("Scripting.Dictionary")
ocnt = -1

Do Until rs2.EOF
  Dim nm, mm, wa, key, arr, cls
  nm = NormName(rs2("zip_name_AUTO")) : If nm = "" Then nm = "AL 프레임"
  mm = NormMM(rs2("blength"))
  wa = Trim("" & rs2("WHICHI_AUTO")) 

  ' 선별
  cls = ""
  If (wa = "8" Or wa = "9" Or wa = "24") Then
    cls = "bg-8-9-24"
  ElseIf (wa = "1" Or wa = "2") Then
    cls = "bg-door12"
  End If

  key = LCase(nm) & "|" & mm & "|" & wa  ' whichi까지 포함해 분리

  If dict.Exists(key) Then
    arr = dict(key)                 ' arr = [name, mm, qty, whichi, cssClass]
    arr(2) = arr(2) + unitQty       ' 수량 누적
    dict(key) = arr
  Else
    ocnt = ocnt + 1
    ReDim Preserve orderKeys(ocnt)  ' 출력 순서 보존
    orderKeys(ocnt) = key
    arr = Array(nm, mm, unitQty, wa, cls)
    dict.Add key, arr
  End If
...
' 랜더 1=길이 2=수량 3=위치오토 4=그룹핑CSS클래스
If dict.Count = 0 Then
%>
  <tr><td colspan="6" class="text-center">데이터가 없습니다.</td></tr>
<%
Else
  ' ===== 도어/픽스 분리 =====
  Dim leftList, rightList, normalList
  Set leftList = Server.CreateObject("Scripting.Dictionary")
  Set rightList = Server.CreateObject("Scripting.Dictionary")
  Set normalList = Server.CreateObject("Scripting.Dictionary")

  For Each key In orderKeys
    arrTmp = dict(key)
    wa = arrTmp(3)

    If (wa="8" Or wa="9" Or wa="24" Or wa="5") Then
        rightList.Add key, arrTmp      ' 픽스류 오른쪽 고정
    Else
        normalList.Add key, arrTmp     ' 나머지 일반 자재
    End If
  Next

  ' ===== 1단계: 픽스류는 오른쪽, 일반 자재는 왼쪽에 먼저 짝 =====
  Dim i, keyN, keyR, arrN, arrR, totalNormal, totalFix
  totalNormal = normalList.Count
  totalFix = rightList.Count
  Dim maxPairCount
  If totalNormal > totalFix Then
      maxPairCount = totalFix
  Else
      maxPairCount = totalNormal
  End If

  ' 픽스류 개수만큼 우선 2열 배치
  For i = 0 To maxPairCount - 1
    keyN = normalList.Keys()(i)
    arrN = normalList(keyN)
    keyR = rightList.Keys()(i)
    arrR = rightList(keyR)

    Response.Write "<tr>"
    ' 왼쪽: 일반
    Response.Write "<td class='name " & arrN(4) & "'>" & Server.HTMLEncode(arrN(0)) & "</td>"
    Response.Write "<td class='text-num " & arrN(4) & "'>" & arrN(1) & "</td>"
    Response.Write "<td class='text-num " & arrN(4) & "'>" & arrN(2)*total_count & "</td>"
    ' 오른쪽: 픽스
    Response.Write "<td class='name " & arrR(4) & "'>" & Server.HTMLEncode(arrR(0)) & "</td>"
    Response.Write "<td class='text-num " & arrR(4) & "'>" & arrR(1) & "</td>"
    Response.Write "<td class='text-num " & arrR(4) & "'>" & arrR(2)*total_count & "</td>"
    Response.Write "</tr>"
  Next

  ' ===== 2단계: 남은 일반 자재는 좌우 번갈아 배치 =====
  Dim remainList, remainCount, j, a1, a2
  remainCount = totalNormal - maxPairCount
  If remainCount > 0 Then
    For j = 0 To remainCount - 1 Step 2
      Response.Write "<tr>"

      ' 왼쪽
      a1 = normalList(normalList.Keys()(maxPairCount + j))
      Response.Write "<td class='name " & a1(4) & "'>" & Server.HTMLEncode(a1(0)) & "</td>"
      Response.Write "<td class='text-num " & a1(4) & "'>" & a1(1) & "</td>"
      Response.Write "<td class='text-num " & a1(4) & "'>" & a1(2)*total_count & "</td>"

      ' 오른쪽 (짝이 있을 경우만)
      If (j + 1) < remainCount Then
        a2 = normalList(normalList.Keys()(maxPairCount + j + 1))
        Response.Write "<td class='name " & a2(4) & "'>" & Server.HTMLEncode(a2(0)) & "</td>"
        Response.Write "<td class='text-num " & a2(4) & "'>" & a2(1) & "</td>"
        Response.Write "<td class='text-num " & a2(4) & "'>" & a2(2)*total_count & "</td>"
      Else
  ...
  ' ===== 3단계: 남은 픽스류(혹시 일반보다 많을 때) =====
  If totalFix > totalNormal Then
    For i = maxPairCount To totalFix - 1
      arrR = rightList(rightList.Keys()(i))
      Response.Write "<tr>"
      Response.Write "<td colspan='3'></td>"
      Response.Write "<td class='name " & arrR(4) & "'>" & Server.HTMLEncode(arrR(0)) & "</td>"
      Response.Write "<td class='text-num " & arrR(4) & "'>" & arrR(1) & "</td>"
  ...
  Set normalList = Nothing
  Set rightList = Nothing
