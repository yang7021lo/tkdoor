/**
 * CRUD Engine - Generic Tabulator CRUD with Dirty Buffer, Batch API, Clipboard Paste
 *
 * 사용법: window.CRUD_CONFIG 설정 후 이 파일 로드
 */
(function(){
"use strict";

var CFG = window.CRUD_CONFIG;
if (!CFG) { console.error("CRUD_CONFIG not defined"); return; }

// === State ===
var table = null;
var dirtyBuffer = {};    // key → {pk, original, current, status}
var tempIdSeq = 0;
var toolbarEl = null;
var _pasteTargetField = null;  // 헤더 클릭으로 선택된 붙여넣기 대상 컬럼
var _lastPastedIds = [];       // 마지막 생성된 행 tempId 목록
var _lastSelectedPos = -1;     // 마지막 선택 행 위치
var _shiftHeld = false;        // Shift 키 상태

// ============================================================
// INIT
// ============================================================
// Shift 키 상태 추적
document.addEventListener("keydown", function(e){ if (e.key === "Shift") _shiftHeld = true; });
document.addEventListener("keyup", function(e){ if (e.key === "Shift") _shiftHeld = false; });

document.addEventListener("DOMContentLoaded", function(){
  createToolbar();
  createTable();
  initShiftSelect();
  initClipboard();
  initKeyboard();
});

// ============================================================
// TOOLBAR
// ============================================================
function createToolbar(){
  var el = document.createElement("div");
  el.className = "crud-toolbar";
  // 붙여넣기 대상 컬럼 옵션 생성
  var pasteOpts = '<option value="">붙여넣기 대상</option>';
  if (CFG.pasteColumns) {
    for (var pi = 0; pi < CFG.pasteColumns.length; pi++) {
      var pf = CFG.pasteColumns[pi];
      var plabel = pf;
      // columns 설정에서 title 찾기
      for (var ci = 0; ci < CFG.columns.length; ci++) {
        if (CFG.columns[ci].field === pf) { plabel = CFG.columns[ci].title; break; }
      }
      pasteOpts += '<option value="' + pf + '">' + plabel + '</option>';
    }
  }

  el.innerHTML =
    '<div class="crud-toolbar-left">' +
      '<button class="crud-btn crud-btn-add" title="행 추가 (Ctrl+I)">+ 행 추가</button>' +
      '<button class="crud-btn crud-btn-del" title="선택 행 삭제 (Del)">삭제 표시</button>' +
      '<button class="crud-btn crud-btn-save" title="변경 저장 (Ctrl+S)">저장</button>' +
      '<button class="crud-btn crud-btn-copy" title="선택 행 복제 (Ctrl+D)">복제</button>' +
      '<button class="crud-btn crud-btn-excel" title="엑셀 다운로드">엑셀</button>' +
      '<button class="crud-btn crud-btn-print" title="인쇄 (Ctrl+P)">출력</button>' +
      '<button class="crud-btn" onclick="location.reload()">새로고침</button>' +
      '<select class="crud-paste-target" title="컬럼 선택 후 Ctrl+V">' + pasteOpts + '</select>' +
      '<input type="text" class="crud-search" placeholder="검색... (Enter)">' +
    '</div>' +
    '<div class="crud-toolbar-right">' +
      '<span class="crud-dirty-count"></span>' +
      '<button class="crud-btn crud-btn-help" title="도움말">사용가이드</button>' +
    '</div>';

  var target = document.querySelector(CFG.tableEl || "#crud-table");
  target.parentNode.insertBefore(el, target);
  toolbarEl = el;

  el.querySelector(".crud-btn-add").onclick = addRow;
  el.querySelector(".crud-btn-del").onclick = markSelectedDeleted;
  el.querySelector(".crud-btn-save").onclick = save;
  el.querySelector(".crud-btn-copy").onclick = duplicateSelected;
  el.querySelector(".crud-btn-excel").onclick = exportExcel;
  el.querySelector(".crud-btn-print").onclick = printTable;
  el.querySelector(".crud-btn-help").onclick = showHelp;
  el.querySelector(".crud-search").onkeydown = function(e){
    if (e.key === "Enter") doSearch(e.target.value);
  };
}

function showHelp(){
  var existing = document.querySelector(".crud-help-overlay");
  if (existing){ existing.remove(); return; }

  var ov = document.createElement("div");
  ov.className = "crud-help-overlay";
  ov.innerHTML =
    '<div class="crud-help-box">' +
      '<div class="crud-help-title">사용 가이드</div>' +
      '<table class="crud-help-table">' +
        '<tr><th colspan="2">편집</th></tr>' +
        '<tr><td>셀 클릭</td><td>직접 편집 (수정된 셀은 노란색 표시)</td></tr>' +
        '<tr><td>+ 행 추가</td><td>빈 행 추가</td></tr>' +
        '<tr><td>삭제 표시</td><td>선택한 행 삭제 표시 (취소선)</td></tr>' +
        '<tr><td>우클릭</td><td>행 삭제/취소/복제 메뉴</td></tr>' +
        '<tr><th colspan="2">선택</th></tr>' +
        '<tr><td>체크박스 클릭</td><td>행 선택/해제</td></tr>' +
        '<tr><td>Shift + 체크박스</td><td>범위 선택 (사이 행 전부)</td></tr>' +
        '<tr><td>헤더 체크박스</td><td>전체 선택/해제</td></tr>' +
        '<tr><th colspan="2">엑셀 붙여넣기</th></tr>' +
        '<tr><td>여러 컬럼</td><td>엑셀에서 복사 → 빈 곳 클릭 → Ctrl+V</td></tr>' +
        '<tr><td>단일 컬럼</td><td>"붙여넣기 대상" 선택 → Ctrl+V<br>(이미 추가된 행에 컬럼별로 채우기)</td></tr>' +
        '<tr><th colspan="2">단축키</th></tr>' +
        '<tr><td>Ctrl + S</td><td>저장</td></tr>' +
        '<tr><td>Ctrl + I</td><td>행 추가</td></tr>' +
        '<tr><td>Delete</td><td>선택 행 삭제 표시</td></tr>' +
        '<tr><th colspan="2">기타</th></tr>' +
        '<tr><td>컬럼 헤더 클릭</td><td>정렬 (오름/내림차순)</td></tr>' +
        '<tr><td>컬럼 헤더 드래그</td><td>컬럼 순서 변경</td></tr>' +
        '<tr><td>검색</td><td>검색어 입력 후 Enter</td></tr>' +
        '<tr><td>새로고침</td><td>변경사항 버리고 서버에서 다시 로드</td></tr>' +
      '</table>' +
      '<button class="crud-btn" onclick="this.parentNode.parentNode.remove()">닫기</button>' +
    '</div>';
  ov.onclick = function(e){ if (e.target === ov) ov.remove(); };
  document.body.appendChild(ov);
}

function updateToolbar(){
  var keys = Object.keys(dirtyBuffer);
  var cnt = keys.length;
  var el = toolbarEl.querySelector(".crud-dirty-count");
  el.textContent = cnt > 0 ? ("변경: " + cnt + "건") : "";
  var btn = toolbarEl.querySelector(".crud-btn-save");
  btn.disabled = cnt === 0;
}

// ============================================================
// TABULATOR
// ============================================================
function createTable(){
  var cols = [];

  // 체크박스 컬럼
  cols.push({
    formatter:"rowSelection", titleFormatter:"rowSelection",
    width:30, headerSort:false, hozAlign:"center", frozen:true
  });

  // 상태 표시 컬럼
  cols.push({
    title:"", field:"_status", width:28, headerSort:false, hozAlign:"center",
    formatter: function(cell){
      var v = cell.getValue();
      if (v === "new") return '<span class="crud-status-new">N</span>';
      if (v === "modified") return '<span class="crud-status-mod">M</span>';
      if (v === "deleted") return '<span class="crud-status-del">D</span>';
      return "";
    }
  });

  // 사용자 컬럼
  for (var i = 0; i < CFG.columns.length; i++){
    var c = CFG.columns[i];
    var col = {
      title: c.title,
      field: c.field,
      headerSort: c.headerSort !== false,
      visible: c.visible !== false
    };
    if (c.width) col.width = c.width;
    if (c.minWidth) col.minWidth = c.minWidth;
    if (c.widthGrow) col.widthGrow = c.widthGrow;
    if (c.hozAlign) col.hozAlign = c.hozAlign;
    if (c.editable !== false && c.editor){
      col.editor = c.editor;
      if (c.editorParams) col.editorParams = c.editorParams;
    }
    if (c.formatter) col.formatter = c.formatter;
    cols.push(col);
  }

  table = new Tabulator(CFG.tableEl || "#crud-table", {
    columns: cols,
    layout: "fitColumns",
    height: CFG.height || "calc(100vh - 160px)",
    renderVertical: "virtual",
    movableColumns: true,
    selectable: true,
    selectableRange: false,

    // Remote Pagination
    pagination: true,
    paginationMode: "remote",
    paginationSize: CFG.pageSize || 50,
    paginationSizeSelector: [25, 50, 100, 200, 500],
    sortMode: "remote",

    ajaxURL: CFG.apiUrl + "?action=list",
    ajaxURLGenerator: function(url, config, params){
      var u = CFG.apiUrl + "?action=list";
      u += "&page=" + (params.page || 1);
      u += "&size=" + (params.size || CFG.pageSize || 50);
      // sorters: Tabulator v6 passes [{column, field, dir}]
      var sorters = params.sorters || params.sort || [];
      if (sorters && sorters.length > 0){
        u += "&sort=" + encodeURIComponent(sorters[0].field || "");
        u += "&dir=" + encodeURIComponent(sorters[0].dir || "desc");
      }
      if (CFG._searchText) u += "&search=" + encodeURIComponent(CFG._searchText);
      console.log("[CRUD] ajaxURLGenerator:", u, "params:", JSON.stringify(params, function(k,v){ return k==="column" ? undefined : v; }));
      return u;
    },
    ajaxResponse: function(url, params, response){
      if (response.error){
        showToast("데이터 로드 오류: " + response.msg, "error");
        return {last_page:1, data:[]};
      }
      return {
        last_page: Math.ceil((response.totalRows || 0) / (response.pageSize || 50)),
        data: response.data || []
      };
    },

    // Events
    rowFormatter: function(row){
      var d = row.getData();
      var el = row.getElement();
      el.classList.remove("crud-row-new", "crud-row-deleted");
      if (d._status === "new") el.classList.add("crud-row-new");
      if (d._status === "deleted") el.classList.add("crud-row-deleted");
    },
    rowContextMenu: [
      {label:"행 삭제 표시", action:function(e,row){ markDeleted(row); }},
      {label:"삭제 취소", action:function(e,row){ unmarkDeleted(row); }},
      {separator:true},
      {label:"행 복제", action:function(e,row){ duplicateRow(row); }}
    ],
    dataLoaded: function(){
      // 페이지 이동 시 dirty buffer에서 현재 페이지에 없는 항목은 유지
    }
  });

  // cellEdited 이벤트 등록 (.on 방식)
  table.on("cellEdited", function(cell){
    console.log("[CRUD] cellEdited:", cell.getField(), "→", cell.getValue());
    onCellEdited(cell);
  });
}

// ============================================================
// DIRTY BUFFER
// ============================================================
function getRowKey(data){
  var pk = data[CFG.pk];
  if (pk && pk !== "" && pk !== "0") return "pk_" + pk;
  if (data._tempId) return data._tempId;
  return null;
}

function onCellEdited(cell){
  var row = cell.getRow();
  var data = row.getData();
  var key = getRowKey(data);
  if (!key) return;

  if (!dirtyBuffer[key]){
    // 처음 수정: 원본 캡처
    var orig = {};
    var fields = row.getCells();
    for (var i = 0; i < fields.length; i++){
      var f = fields[i].getField();
      if (f && f.charAt(0) !== "_") orig[f] = fields[i].getInitialValue();
    }
    dirtyBuffer[key] = {
      pk: data[CFG.pk] || null,
      original: orig,
      current: shallowCopy(data),
      status: data._status === "new" ? "new" : "modified"
    };
  } else {
    dirtyBuffer[key].current = shallowCopy(data);
  }

  // 원복 체크 (modified만)
  if (dirtyBuffer[key].status === "modified"){
    if (isEqualData(dirtyBuffer[key].original, dirtyBuffer[key].current)){
      delete dirtyBuffer[key];
      row.update({_status:""});
      row.reformat();
      updateToolbar();
      return;
    }
  }

  row.update({_status: dirtyBuffer[key].status});
  row.reformat();
  highlightDirtyCells(row);
  updateToolbar();

  // 셀 편집 완료 시 자동 저장 (디바운스)
  clearTimeout(onCellEdited._timer);
  onCellEdited._timer = setTimeout(function(){ save(); }, 400);
}

function highlightDirtyCells(row){
  var data = row.getData();
  var key = getRowKey(data);
  var entry = dirtyBuffer[key];
  var cells = row.getCells();
  for (var i = 0; i < cells.length; i++){
    var f = cells[i].getField();
    if (!f || f.charAt(0) === "_") continue;
    var el = cells[i].getElement();
    if (entry && entry.original[f] !== undefined &&
        String(entry.original[f]) !== String(entry.current[f])){
      el.classList.add("crud-cell-dirty");
    } else {
      el.classList.remove("crud-cell-dirty");
    }
  }
}

// ============================================================
// ROW OPERATIONS
// ============================================================
function addRow(){
  tempIdSeq++;
  var tempId = "_new_" + tempIdSeq;
  var row = {_status:"new", _tempId:tempId};
  // 기본값
  for (var i = 0; i < CFG.columns.length; i++){
    var c = CFG.columns[i];
    if (c.defaultValue !== undefined) row[c.field] = c.defaultValue;
  }
  table.addRow(row, true);
  dirtyBuffer[tempId] = {
    pk:null, original:{}, current:shallowCopy(row), status:"new"
  };
  updateToolbar();
}

function markDeleted(row){
  var data = row.getData();
  var pk = data[CFG.pk];

  // 신규 행은 바로 제거
  if (!pk || pk === "" || pk === "0"){
    var tid = data._tempId;
    if (tid && dirtyBuffer[tid]) delete dirtyBuffer[tid];
    row.delete();
    updateToolbar();
    return;
  }

  var key = "pk_" + pk;
  if (!dirtyBuffer[key]){
    dirtyBuffer[key] = {
      pk:pk, original:shallowCopy(data), current:shallowCopy(data), status:"deleted"
    };
  } else {
    dirtyBuffer[key].status = "deleted";
  }
  row.update({_status:"deleted"});
  row.reformat();
  updateToolbar();
}

function unmarkDeleted(row){
  var data = row.getData();
  var pk = data[CFG.pk];
  if (!pk) return;
  var key = "pk_" + pk;
  if (!dirtyBuffer[key]) return;

  if (isEqualData(dirtyBuffer[key].original, dirtyBuffer[key].current)){
    delete dirtyBuffer[key];
    row.update({_status:""});
  } else {
    dirtyBuffer[key].status = "modified";
    row.update({_status:"modified"});
  }
  row.reformat();
  updateToolbar();
}

function markSelectedDeleted(){
  var rows = table.getSelectedRows();
  if (!rows.length){ showToast("행을 선택하세요.", "info"); return; }
  for (var i = 0; i < rows.length; i++) markDeleted(rows[i]);
}

function duplicateRow(row){
  var data = shallowCopy(row.getData());
  delete data[CFG.pk];
  delete data._status;
  delete data._tempId;
  tempIdSeq++;
  data._tempId = "_new_" + tempIdSeq;
  data._status = "new";
  table.addRow(data, true);
  dirtyBuffer[data._tempId] = {
    pk:null, original:{}, current:shallowCopy(data), status:"new"
  };
  updateToolbar();
}

// ============================================================
// EXCEL EXPORT
// ============================================================
function exportExcel(){
  var rows = table.getSelectedRows();
  if (!rows.length) rows = table.getRows(); // 선택 없으면 전체

  // 사용자 컬럼만 (내부 컬럼 제외)
  var cols = [];
  for (var i = 0; i < CFG.columns.length; i++){
    if (CFG.columns[i].visible !== false) cols.push(CFG.columns[i]);
  }

  // HTML 테이블 생성
  var html = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel">';
  html += '<head><meta charset="utf-8"><style>td,th{border:1px solid #ccc;padding:4px 8px;font-size:12px;} th{background:#374151;color:#fff;font-weight:bold;}</style></head>';
  html += '<body><table>';

  // 헤더
  html += '<tr>';
  for (var c = 0; c < cols.length; c++){
    html += '<th>' + escHtml(cols[c].title) + '</th>';
  }
  html += '</tr>';

  // 데이터
  for (var r = 0; r < rows.length; r++){
    var data = rows[r].getData();
    if (data._status === "deleted") continue;
    html += '<tr>';
    for (var c = 0; c < cols.length; c++){
      var val = data[cols[c].field];
      if (val === null || val === undefined) val = "";
      // formatter가 있으면 표시값 사용
      if (cols[c].formatter && typeof cols[c].formatter === "function"){
        try {
          var display = cols[c].formatter({getValue:function(){return val;}});
          if (display) val = display;
        } catch(e){}
      }
      html += '<td>' + escHtml(String(val)) + '</td>';
    }
    html += '</tr>';
  }

  html += '</table></body></html>';

  // 다운로드
  var blob = new Blob(['\ufeff' + html], {type:'application/vnd.ms-excel;charset=utf-8'});
  var url = URL.createObjectURL(blob);
  var a = document.createElement('a');
  a.href = url;
  a.download = (document.title || 'export') + '.xls';
  document.body.appendChild(a);
  a.click();
  setTimeout(function(){ document.body.removeChild(a); URL.revokeObjectURL(url); }, 100);

  showToast((rows.length) + "건 엑셀 다운로드", "success");
}

// ============================================================
// PRINT
// ============================================================
function printTable(){
  var rows = table.getSelectedRows();
  if (!rows.length) rows = table.getRows();

  var cols = [];
  for (var i = 0; i < CFG.columns.length; i++){
    if (CFG.columns[i].visible !== false) cols.push(CFG.columns[i]);
  }

  var html = '<!DOCTYPE html><html><head><meta charset="utf-8">';
  html += '<title>' + (document.title || '출력') + '</title>';
  html += '<style>';
  html += 'body{font-family:"Malgun Gothic",sans-serif;font-size:12px;margin:20px;}';
  html += 'h2{font-size:16px;margin-bottom:10px;}';
  html += 'table{border-collapse:collapse;width:100%;}';
  html += 'th,td{border:1px solid #999;padding:4px 8px;text-align:center;font-size:11px;}';
  html += 'th{background:#374151;color:#fff;font-weight:bold;}';
  html += 'tr:nth-child(even){background:#f9fafb;}';
  html += '.print-info{font-size:11px;color:#666;margin-bottom:8px;}';
  html += '@media print{body{margin:10px;} .no-print{display:none;}}';
  html += '</style></head><body>';
  html += '<h2>' + (document.title || '') + '</h2>';
  html += '<div class="print-info">출력일: ' + new Date().toLocaleString('ko-KR') + ' | 총 ' + rows.length + '건</div>';
  html += '<table><thead><tr>';

  for (var c = 0; c < cols.length; c++){
    html += '<th>' + escHtml(cols[c].title) + '</th>';
  }
  html += '</tr></thead><tbody>';

  for (var r = 0; r < rows.length; r++){
    var data = rows[r].getData();
    if (data._status === "deleted") continue;
    html += '<tr>';
    for (var c = 0; c < cols.length; c++){
      var val = data[cols[c].field];
      if (val === null || val === undefined) val = "";
      if (cols[c].formatter && typeof cols[c].formatter === "function"){
        try {
          var display = cols[c].formatter({getValue:function(){return val;}});
          if (display) val = display;
        } catch(e){}
      }
      html += '<td>' + escHtml(String(val)) + '</td>';
    }
    html += '</tr>';
  }

  html += '</tbody></table></body></html>';

  var win = window.open('', '_blank');
  win.document.write(html);
  win.document.close();
  win.focus();
  win.print();
}

function escHtml(s){
  return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
}

function duplicateSelected(){
  var rows = table.getSelectedRows();
  if (!rows.length){ showToast("복제할 행을 선택하세요.", "info"); return; }
  for (var i = 0; i < rows.length; i++) duplicateRow(rows[i]);
  showToast(rows.length + "건 복제 완료", "success");
}

// ============================================================
// CLIPBOARD PASTE
// ============================================================
function getSelectedPasteTarget(){
  var sel = toolbarEl.querySelector(".crud-paste-target");
  return sel ? sel.value : "";
}

function initClipboard(){
  document.addEventListener("paste", function(e){
    // 편집 중이면 무시 (input/textarea) - 단, 붙여넣기 대상 select는 제외
    var tag = (e.target.tagName || "").toLowerCase();
    if (tag === "input" || tag === "textarea") return;

    var text = (e.clipboardData || window.clipboardData).getData("text");
    if (!text || !text.trim()) return;
    // 탭 또는 줄바꿈 있으면 엑셀 데이터로 처리
    if (text.indexOf("\t") < 0 && text.indexOf("\n") < 0) return;

    e.preventDefault();
    var targetField = getSelectedPasteTarget();
    handlePaste(text, targetField);
  });
}

function handlePaste(text, targetField){
  var pasteCols = CFG.pasteColumns;
  if (!pasteCols || !pasteCols.length){
    showToast("pasteColumns 설정이 없습니다.", "error");
    return;
  }

  var lines = text.split(/\r?\n/).filter(function(l){ return l.trim() !== ""; });
  if (lines.length === 0) return;
  var hasTabs = text.indexOf("\t") >= 0;

  // === 대상 컬럼이 선택된 경우: 기존 신규행에 채우기 ===
  if (targetField && !hasTabs && _lastPastedIds.length > 0) {
    var filled = 0;
    var rows = table.getRows();
    for (var i = 0; i < lines.length && i < _lastPastedIds.length; i++){
      for (var r = 0; r < rows.length; r++){
        var rd = rows[r].getData();
        if (rd._tempId === _lastPastedIds[i]){
          var upd = {};
          upd[targetField] = lines[i].trim();
          rows[r].update(upd);
          // dirty buffer 업데이트
          if (dirtyBuffer[_lastPastedIds[i]]){
            dirtyBuffer[_lastPastedIds[i]].current[targetField] = lines[i].trim();
          }
          filled++;
          break;
        }
      }
    }
    // 컬럼 title 찾기
    var colTitle = targetField;
    for (var ci = 0; ci < CFG.columns.length; ci++){
      if (CFG.columns[ci].field === targetField){ colTitle = CFG.columns[ci].title; break; }
    }
    showToast(colTitle + " " + filled + "건 채움", "info");
    updateToolbar();
    return;
  }

  // === 대상 컬럼이 선택된 경우 + 신규행 없음: 새 행 생성 후 해당 컬럼 채우기 ===
  if (targetField && !hasTabs && _lastPastedIds.length === 0) {
    if (lines.length > 500){
      if (!confirm(lines.length + "행을 생성하시겠습니까?")) return;
    }
    _lastPastedIds = [];
    var newRows = [];
    for (var i = 0; i < lines.length; i++){
      tempIdSeq++;
      var tempId = "_new_" + tempIdSeq;
      var row = {_status:"new", _tempId:tempId};
      row[targetField] = lines[i].trim();
      newRows.push(row);
      _lastPastedIds.push(tempId);
      dirtyBuffer[tempId] = {pk:null, original:{}, current:shallowCopy(row), status:"new"};
    }
    if (newRows.length > 0){
      table.addData(newRows, true);
      updateToolbar();
      var colTitle = targetField;
      for (var ci = 0; ci < CFG.columns.length; ci++){
        if (CFG.columns[ci].field === targetField){ colTitle = CFG.columns[ci].title; break; }
      }
      showToast(newRows.length + "행 생성 + " + colTitle + " 채움", "info");
    }
    return;
  }

  // === 기본: 다중 컬럼 붙여넣기 (탭 구분) ===
  if (lines.length > 500){
    if (!confirm(lines.length + "행을 붙여넣기 하시겠습니까?")) return;
  }

  _lastPastedIds = [];
  var newRows = [];
  for (var i = 0; i < lines.length; i++){
    var cells = lines[i].split("\t");
    tempIdSeq++;
    var tempId = "_new_" + tempIdSeq;
    var row = {_status:"new", _tempId:tempId};

    for (var j = 0; j < pasteCols.length && j < cells.length; j++){
      row[pasteCols[j]] = cells[j].trim();
    }

    if (CFG.onRowAdded){
      var result = CFG.onRowAdded(row);
      if (result === false) continue;
      if (result) row = result;
    }

    newRows.push(row);
    _lastPastedIds.push(tempId);
    dirtyBuffer[tempId] = {
      pk:null, original:{}, current:shallowCopy(row), status:"new"
    };
  }

  if (newRows.length > 0){
    table.addData(newRows, true);
    updateToolbar();
    showToast(newRows.length + "행 붙여넣기 완료 (저장 버튼을 눌러주세요)", "info");
  }
}

// ============================================================
// BATCH SAVE
// ============================================================
function save(){
  var keys = Object.keys(dirtyBuffer);
  if (keys.length === 0){
    showToast("변경사항이 없습니다.", "info");
    return;
  }

  var batch = collectBatchData();

  if (CFG.onBeforeSave){
    batch = CFG.onBeforeSave(batch);
    if (batch === false) return;
  }

  // 변경된 행 녹색 깜빡임
  flashDirtyRows();
  showLoading(true);

  fetch(CFG.apiUrl + "?action=batch", {
    method: "POST",
    headers: {"Content-Type":"application/json; charset=utf-8"},
    body: JSON.stringify(batch)
  })
  .then(function(r){ return r.json(); })
  .then(function(res){
    showLoading(false);
    if (res.result === "ok"){
      showToast(
        "저장 완료: 추가 " + (res.inserted||0) + "건, 수정 " + (res.updated||0) + "건, 삭제 " + (res.deleted||0) + "건",
        "success"
      );
      dirtyBuffer = {};
      _lastPastedIds = [];
      updateToolbar();
      table.setData(); // 서버에서 다시 로드
      if (CFG.onAfterSave) CFG.onAfterSave(res);
    } else {
      showToast("저장 실패: " + (res.msg || "알 수 없는 오류"), "error");
    }
  })
  .catch(function(err){
    showLoading(false);
    showToast("네트워크 오류: " + err.message, "error");
  });
}

function collectBatchData(){
  var inserts = [], updates = [], deletes = [];

  for (var key in dirtyBuffer){
    if (!dirtyBuffer.hasOwnProperty(key)) continue;
    var entry = dirtyBuffer[key];
    var clean = stripInternal(entry.current);

    switch(entry.status){
      case "new":
        inserts.push(clean);
        break;
      case "modified":
        // PK + 변경된 필드만
        var changed = {};
        changed[CFG.pk] = entry.pk;
        for (var f in entry.current){
          if (!entry.current.hasOwnProperty(f)) continue;
          if (f.charAt(0) === "_") continue;
          if (String(entry.original[f]) !== String(entry.current[f])){
            changed[f] = entry.current[f];
          }
        }
        updates.push(changed);
        break;
      case "deleted":
        var d = {};
        d[CFG.pk] = entry.pk;
        deletes.push(d);
        break;
    }
  }

  return {insert:inserts, update:updates, delete:deletes};
}

// ============================================================
// SHIFT+CLICK 범위 선택
// ============================================================
function initShiftSelect(){
  table.on("rowSelected", function(row){
    if (_shiftHeld && _lastSelectedPos > -1) {
      var rows = table.getRows();
      var curPos = row.getPosition(true) - 1;
      var from = Math.min(_lastSelectedPos, curPos);
      var to = Math.max(_lastSelectedPos, curPos);
      for (var ri = from; ri <= to; ri++){
        if (rows[ri]) rows[ri].select();
      }
    }
    _lastSelectedPos = row.getPosition(true) - 1;
  });
  table.on("rowDeselected", function(row){
    _lastSelectedPos = -1;
  });
}

// ============================================================
// SEARCH
// ============================================================
function doSearch(text){
  CFG._searchText = text;
  table.setData(); // 서버 리로드 with search param
}

// ============================================================
// KEYBOARD SHORTCUTS
// ============================================================
function initKeyboard(){
  document.addEventListener("keydown", function(e){
    if ((e.ctrlKey || e.metaKey) && e.key === "s"){
      e.preventDefault();
      save();
    }
    if ((e.ctrlKey || e.metaKey) && e.key === "i"){
      e.preventDefault();
      addRow();
    }
    if ((e.ctrlKey || e.metaKey) && e.key === "d"){
      e.preventDefault();
      duplicateSelected();
    }
    if (e.key === "Delete" && !isEditing(e.target)){
      markSelectedDeleted();
    }
  });
}

function isEditing(el){
  var tag = (el.tagName || "").toLowerCase();
  return tag === "input" || tag === "textarea" || el.contentEditable === "true";
}

// ============================================================
// UI HELPERS
// ============================================================
function flashDirtyRows(){
  var rows = table.getRows();
  for (var i = 0; i < rows.length; i++){
    var data = rows[i].getData();
    var key = getRowKey(data);
    if (key && dirtyBuffer[key]){
      var el = rows[i].getElement();
      el.classList.add("crud-row-saving");
    }
  }
  // 애니메이션 끝나면 클래스 제거
  setTimeout(function(){
    var els = document.querySelectorAll(".crud-row-saving");
    for (var i = 0; i < els.length; i++) els[i].classList.remove("crud-row-saving");
  }, 1300);
}

function showToast(msg, type){
  var el = document.createElement("div");
  el.className = "crud-toast " + (type || "info");
  el.textContent = msg;
  document.body.appendChild(el);
  setTimeout(function(){
    el.style.opacity = "0";
    el.style.transition = "opacity 0.3s";
    setTimeout(function(){ el.remove(); }, 300);
  }, 3000);
}

function showLoading(on){
  var el = document.querySelector(".crud-loading");
  if (!el){
    el = document.createElement("div");
    el.className = "crud-loading";
    el.textContent = "저장 중...";
    var wrap = document.querySelector(CFG.tableEl || "#crud-table");
    wrap.parentNode.style.position = "relative";
    wrap.parentNode.insertBefore(el, wrap);
  }
  el.classList.toggle("show", on);
}

// ============================================================
// UTILITY
// ============================================================
function shallowCopy(obj){
  var o = {};
  for (var k in obj) if (obj.hasOwnProperty(k)) o[k] = obj[k];
  return o;
}

function stripInternal(obj){
  var o = {};
  for (var k in obj){
    if (obj.hasOwnProperty(k) && k.charAt(0) !== "_") o[k] = obj[k];
  }
  return o;
}

function isEqualData(a, b){
  for (var k in a){
    if (k.charAt(0) === "_") continue;
    if (String(a[k] || "") !== String(b[k] || "")) return false;
  }
  return true;
}

// Export
window.CrudEngine = {
  getTable: function(){ return table; },
  getDirty: function(){ return dirtyBuffer; },
  addRow: addRow,
  save: save,
  reload: function(){ dirtyBuffer = {}; _lastPastedIds = []; updateToolbar(); table.setData(); }
};

})();
