  ...
  <div class="d-flex flex-wrap gap-2 align-items-center" style="max-width:100%;">
   <div id="select-area"></div>
    ...
    <button type="button" class="btn btn-success"
        onclick="location.href='/TNG_WMS/DASHBOARD/TNG_WMS_DASHBOARD.asp?ymd=<%=ymd%>&sjcidx=<%=rsjcidx%>&sjmidx=<%=rsjmidx%>'">
        출하대시보드
    </button>
  </div>
</form>
...
function openManualPopup(djmanual_idx){
    const ymd = document.getElementById('ymd')?.value || '';
    const url = 'PopUp/TNG_WMS_DJ_Dashboard_Manual_Popup.asp?ymd=' + encodeURIComponent(ymd) + '&djmanual_idx=' + djmanual_idx;
    window.open(url, 'djManualPop', 'width=760,height=820,scrollbars=yes,resizable=yes');
}
...
SQLM = ""
SQLM = SQLM & "SELECT djmanual_idx, ymd, cname, item_name, meas_name, qtyname, pname, "
SQLM = SQLM & "       coat, djnum, totalquan, spec_text, djmemo "
SQLM = SQLM & "FROM dbo.tk_wms_dj_dashboard_manual WITH (NOLOCK) "
SQLM = SQLM & "WHERE is_active = 1 AND ymd = '" & Replace(ymd,"'","''") & "' "
SQLM = SQLM & "ORDER BY djnum, djmanual_idx"
RsM.Open SQLM, DbCon, 1, 1
Do Until RsM.EOF
    rowNo = rowNo + 1
    Dim mCname, mItem, mMeas, mQty, mPname, mCoat, mDjnum, mTotal, mSpec, mMemo
    Dim mItemDisplay, mSpecHtml, mMemoHtml, mCoatText
    mCname = "" & RsM("cname")
    mItem  = "" & RsM("item_name")
    mMeas  = "" & RsM("meas_name")
    mQty   = "" & RsM("qtyname")
    ...
    mCoat  = RsM("coat")
    mDjnum = "" & RsM("djnum")
    mTotal = CLng(0 & RsM("totalquan"))
    mSpec  = "" & RsM("spec_text")
    mMemo  = "" & RsM("djmemo")
    actual_dt = "" & RsM("ymd")
    If Trim(mMeas) <> "" Then
        mItemDisplay = Server.HTMLEncode(mItem) 
    Else
        mItemDisplay = Server.HTMLEncode(mItem)
    End If
    mSpecHtml = Replace(Server.HTMLEncode(mSpec), vbCrLf, "<br>")
    mMemoHtml = Replace(Server.HTMLEncode(mMemo), vbCrLf, "<br>")
    Select Case CLng(0 & mCoat)
        Case 0: mCoatText = "❌"
        Case 1: mCoatText = "기본(2코트)"
        Case 2: mCoatText = "필수(3코트)"
        Case Else: mCoatText = "?"
    End Select
    actual_dt = Left(actual_dt,4) & "-" & Mid(actual_dt,5,2) & "-" & Mid(actual_dt,7,2)
    trHTML = "<tr>"
    trHTML = trHTML & "<td class='col-center'><span class='djnum-soft'>" & rowNo & "</span></td>"
    trHTML = trHTML & "<td class='col-left'><span class='pnamebold'>" & Server.HTMLEncode(mQty) & "<br>" & Server.HTMLEncode(mPname) & "</span></td>"
    trHTML = trHTML & "<td class='col-center'><span class='djnum-soft'>" & mCoatText & "</span></td>"
    trHTML = trHTML & "<td class='col-center'><button type='button' class='btn btn-sm btn-warning' onclick=""window.open('PopUp/TNG_WMS_DJ_Dashboard_Manual_Popup.asp?djmanual_idx=" & RsM("djmanual_idx") & "','edit','width=760,height=820,scrollbars=yes');"">수정</button></td>"
    trHTML = trHTML & "<td class='col-center'><span class='djnum-soft'>" & Server.HTMLEncode(mDjnum) & "</span></td>"
    If mTotal > 0 Then
        trHTML = trHTML & "<td class='col-center'><span class='totalbold'>" & mTotal & "개</span></td>"
    Else
        trHTML = trHTML & "<td></td>"
    End If
    If mSpecHtml <> "" Then
        trHTML = trHTML & "<td>" & mSpecHtml & "</td>"
    Else
        trHTML = trHTML & "<td></td>"
    End If
    trHTML = trHTML & "<td class='col-center'><span class='spbold'>" & Server.HTMLEncode(mCname) & "</span></td>"
    trHTML = trHTML & "<td class='col-left'><span class='spbold'>" & mItemDisplay & "</span></td>"
    trHTML = trHTML & "<td class='col-left'><span class='spbold' style='font-size:18px'>" & actual_dt & "</span></td>"
    trHTML = trHTML & "<td class='col-left'>" & _
        "<br>" & mMemoHtml & "</td>"
    trHTML = trHTML & "</tr>"
...
If Not (Rs.BOF Or Rs.EOF) Then
    Do Until Rs.EOF
        ...
        If dictDetail.Exists(cur_sjsidx) Then
            Response.Write "<!-- dictDetail OK : " & cur_sjsidx & " -->"
        Else
            Response.Write "<!-- dictDetail EMPTY : " & cur_sjsidx & " -->"
        End If
        ...
        ' 출고일자
        trHTML = trHTML & "<td class='col-left'><span class='spbold' style='font-size:18px'>" & actual_ship_dt & "</span></td>"

        ' 비고 (도장번호 메모)
        If djmemo <> "" Then
            trHTML = trHTML & "<td class='col-left' onclick=""window.open('Popup/TNG_WMS_Bigo_Djpopup.asp?sjsidx=" & cur_sjsidx & "','move','width=1300,height=520,scrollbars=yes');"" style='cursor:pointer; font-weight:bold; white-space:pre-line; line-height:1.4;'><span class='spbold'>" & djmemo & "</span></td>"
        Else
            trHTML = trHTML & "<td class='col-left'onclick=""window.open('Popup/TNG_WMS_Bigo_Djpopup.asp?sjsidx=" & cur_sjsidx & "','move','width=1300,height=520,scrollbars=yes');""></td>"
        End If
...
<tr style="background:#eef4ff;">
    ...
    <th style="width:25%;">규격 (길이 × 수량)</th>
    <th style="width:10%;">거래처명</th>
    <th style="width:12%;">품목명(검측)</th>
    <th style="width:9%;">출고일자</th>
